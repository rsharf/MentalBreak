-- Instance Portal for Siren's Grotto
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "sirens"
local ZONE_ID = 125
local LOCKOUT = "inst_sirens_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = -33.0, 196.0, 4.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Siren's Grotto where all named creatures are waiting. Say 'enter' when you are ready.")
