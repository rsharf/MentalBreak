-- Instance Portal for The Permafrost Caverns
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "permafrost"
local ZONE_ID = 73
local LOCKOUT = "inst_permafrost_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 61.0, -121.0, 2.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of The Permafrost Caverns where all named creatures are waiting. Say 'enter' when you are ready.")
