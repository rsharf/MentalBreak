-- Crushbone Instance Encounter (version 100)
-- Named NPCs spawn via database spawn2 (version=100)
-- Lockout set on #Orc_Messenger (ID 58060) death

function Boss_Death(e)
    eq.set_global("inst_crushbone_lockout", tostring(os.time()), 2, "H24")
...
function Zone_Entry(e)
    if eq.get_zone_instance_version() == 100 then
        e.self:GMMove(158.0, -644.0, 4.0, 0)
    end
end

function event_encounter_load(e)
    eq.register_npc_event('instance_boss', Event.death_complete, 58060, Boss_Death)
    eq.register_npc_event('instance_boss', Event.combat, 58060, Boss_Combat)
