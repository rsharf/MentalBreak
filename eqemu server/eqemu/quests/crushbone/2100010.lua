-- Instance Portal for Crushbone
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "crushbone"
local ZONE_ID = 58
local LOCKOUT = "inst_crushbone_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 158.0, -644.0, 4.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Crushbone where all named creatures are waiting. Say 'enter' when you are ready.")
