-- Instance Portal for The Northern Plains of Karana
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "northkarana"
local ZONE_ID = 13
local LOCKOUT = "inst_northkarana_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = -382.0, -284.0, -8.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of The Northern Plains of Karana where all named creatures are waiting. Say 'enter' when you are ready.")
