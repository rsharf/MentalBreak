-- The Northern Plains of Karana Instance Encounter (version 100)
-- Named NPCs spawn via database spawn2 (version=100)
-- Lockout set on #Sir_Gerwin_Thunderblade (ID 13125) death

function Boss_Death(e)
    eq.set_global("inst_northkarana_lockout", tostring(os.time()), 2, "H24")
...
function Zone_Entry(e)
    if eq.get_zone_instance_version() == 100 then
        e.self:GMMove(-382.0, -284.0, -8.0, 0)
    end
end

function event_encounter_load(e)
    eq.register_npc_event('instance_boss', Event.death_complete, 13125, Boss_Death)
    eq.register_npc_event('instance_boss', Event.combat, 13125, Boss_Combat)
