-- Instance Portal for Innothule Swamp
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "innothule"
local ZONE_ID = 46
local LOCKOUT = "inst_innothule_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = -588.0, -2192.0, -25.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Innothule Swamp where all named creatures are waiting. Say 'enter' when you are ready.")
