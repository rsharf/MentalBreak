-- Instance Portal for Nedaria's Landing
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "nedaria"
local ZONE_ID = 182
local LOCKOUT = "inst_nedaria_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = -1737.0, -181.0, 256.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Nedaria's Landing where all named creatures are waiting. Say 'enter' when you are ready.")
