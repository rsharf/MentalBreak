-- Instance Portal for Kithicor Forest
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "kithicor"
local ZONE_ID = 20
local LOCKOUT = "inst_kithicor_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 3828.0, 1889.0, 459.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Kithicor Forest where all named creatures are waiting. Say 'enter' when you are ready.")
