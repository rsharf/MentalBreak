-- Instance Portal for Frontier Mountains
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "frontiermtns"
local ZONE_ID = 92
local LOCKOUT = "inst_frontiermtns_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = -4262.0, -633.0, 116.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Frontier Mountains where all named creatures are waiting. Say 'enter' when you are ready.")
