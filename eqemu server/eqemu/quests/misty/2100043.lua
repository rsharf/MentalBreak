-- Instance Portal for Misty Thicket
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "misty"
local ZONE_ID = 33
local LOCKOUT = "inst_misty_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 0.0, 0.0, 2.43

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Misty Thicket where all named creatures are waiting. Say 'enter' when you are ready.")
