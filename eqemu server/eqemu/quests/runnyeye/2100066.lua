-- Instance Portal for The Liberated Citadel of Runnyeye
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "runnyeye"
local ZONE_ID = 11
local LOCKOUT = "inst_runnyeye_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 201.0, 90.0, 4.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of The Liberated Citadel of Runnyeye where all named creatures are waiting. Say 'enter' when you are ready.")
