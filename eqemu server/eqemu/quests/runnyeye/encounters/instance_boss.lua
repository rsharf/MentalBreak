-- The Liberated Citadel of Runnyeye Instance Encounter (version 100)
-- Named NPCs spawn via database spawn2 (version=100)
-- Lockout set on #Lord_Pickclaw (ID 11017) death

function Boss_Death(e)
    eq.set_global("inst_runnyeye_lockout", tostring(os.time()), 2, "H24")
...
function Zone_Entry(e)
    if eq.get_zone_instance_version() == 100 then
        e.self:GMMove(201.0, 90.0, 4.0, 0)
    end
end

function event_encounter_load(e)
    eq.register_npc_event('instance_boss', Event.death_complete, 11017, Boss_Death)
    eq.register_npc_event('instance_boss', Event.combat, 11017, Boss_Combat)
