-- Instance Portal for The Warsliks Woods
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "warslikswood"
local ZONE_ID = 79
local LOCKOUT = "inst_warslikswood_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = -468.0, -1429.0, 198.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of The Warsliks Woods where all named creatures are waiting. Say 'enter' when you are ready.")
