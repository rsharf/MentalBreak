-- Instance Portal for North Qeynos
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "qeynos2"
local ZONE_ID = 2
local LOCKOUT = "inst_qeynos2_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = -74.0, 428.0, 3.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of North Qeynos where all named creatures are waiting. Say 'enter' when you are ready.")
