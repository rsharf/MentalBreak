-- Instance Portal for Southern Felwithe
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "felwitheb"
local ZONE_ID = 62
local LOCKOUT = "inst_felwitheb_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = -790.0, 320.0, -10.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Southern Felwithe where all named creatures are waiting. Say 'enter' when you are ready.")
