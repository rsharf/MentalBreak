-- Instance Portal for Solusek's Eye
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "soldunga"
local ZONE_ID = 31
local LOCKOUT = "inst_soldunga_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = -486.0, -476.0, 73.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Solusek's Eye where all named creatures are waiting. Say 'enter' when you are ready.")
