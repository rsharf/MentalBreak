-- Instance Portal for South Kaladim
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "kaladima"
local ZONE_ID = 60
local LOCKOUT = "inst_kaladima_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = -2.0, -18.0, 3.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of South Kaladim where all named creatures are waiting. Say 'enter' when you are ready.")
