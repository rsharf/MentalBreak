...
sub EVENT_LEVEL_UP {
    # NOTE: Multiclass spell scribing is handled by global_player.lua event_level_up,
    # which takes priority over this Perl handler. Do not duplicate that logic here.
    # This handler only runs if the Lua version is absent or disabled.
    my $level = $ulevel;
    $client->ScribeSpells(1, $level);
    $client->LearnDisciplines(1, $level);
    $client->Save();
}
