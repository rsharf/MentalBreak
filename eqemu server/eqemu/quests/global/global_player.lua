...
function event_connect(e)
	...
	local qglobals = eq.get_qglobals(e.self)
	if qglobals.origin_granted == nil then
...
function event_level_up(e)
  ...
  -- Max all skills for ALL classes (multiclass-aware via MergedSkillCap)
  e.self:MaxSkills()

  -- Auto-grant class specific AAs
  grant_class_aas(e)

  -- Auto-scribe spells and disciplines for ALL classes (multiclass-aware in C++ via ForEachClass)
  local spells_scribed = e.self:ScribeSpells(1, level)
  local discs_learned = e.self:LearnDisciplines(1, level)

  if spells_scribed > 0 then
    local word = spells_scribed == 1 and "spell" or "spells"
    e.self:Message(15, "You have learned " .. spells_scribed .. " new " .. word .. "!")
  end
  if discs_learned > 0 then
    local word = discs_learned == 1 and "discipline" or "disciplines"
    e.self:Message(15, "You have learned " .. discs_learned .. " new " .. word .. "!")
  end
  ...
  if level == 5 then
    -- Origin is now auto-granted at level 1 on creation, so this popup is no longer needed.
  end

  if level == 10 and eq.is_dragons_of_norrath_enabled() then
