-- Instance Portal for The Mines of Gloomingdeep
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "tutorialb"
local ZONE_ID = 189
local LOCKOUT = "inst_tutorialb_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 18.0, -147.0, 20.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of The Mines of Gloomingdeep where all named creatures are waiting. Say 'enter' when you are ready.")
