-- Instance Portal for Timorous Deep
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "timorous"
local ZONE_ID = 96
local LOCKOUT = "inst_timorous_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 2194.0, -5392.0, 6.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Timorous Deep where all named creatures are waiting. Say 'enter' when you are ready.")
