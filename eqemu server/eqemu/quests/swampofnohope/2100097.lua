-- Instance Portal for The Swamp of No Hope
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "swampofnohope"
local ZONE_ID = 83
local LOCKOUT = "inst_swampofnohope_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 2945.0, 2761.0, 6.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of The Swamp of No Hope where all named creatures are waiting. Say 'enter' when you are ready.")
