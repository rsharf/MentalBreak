-- Instance Portal for The Emerald Jungle
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "emeraldjungle"
local ZONE_ID = 94
local LOCKOUT = "inst_emeraldjungle_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 4648.0, -1223.0, 2.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of The Emerald Jungle where all named creatures are waiting. Say 'enter' when you are ready.")
