-- Instance Portal for The Temple of Solusek Ro
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "soltemple"
local ZONE_ID = 80
local LOCKOUT = "inst_soltemple_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 36.0, 262.0, 0.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of The Temple of Solusek Ro where all named creatures are waiting. Say 'enter' when you are ready.")
