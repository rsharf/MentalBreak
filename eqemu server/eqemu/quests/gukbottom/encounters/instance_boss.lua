-- The Ruins of Old Guk Instance Encounter (version 100)
-- Named NPCs spawn via database spawn2 (version=100)
-- Lockout set on the_ghoul_lord (ID 66005) death

function Boss_Death(e)
    eq.set_global("inst_gukbottom_lockout", tostring(os.time()), 2, "H24")
...
function Zone_Entry(e)
    if eq.get_zone_instance_version() == 100 then
        e.self:GMMove(-217.0, 1197.0, -78.0, 0)
    end
end

function event_encounter_load(e)
    eq.register_npc_event('instance_boss', Event.death_complete, 66005, Boss_Death)
    eq.register_npc_event('instance_boss', Event.combat, 66005, Boss_Combat)
