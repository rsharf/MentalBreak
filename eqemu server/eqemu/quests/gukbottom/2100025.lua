-- Instance Portal for The Ruins of Old Guk
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "gukbottom"
local ZONE_ID = 66
local LOCKOUT = "inst_gukbottom_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = -217.0, 1197.0, -78.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of The Ruins of Old Guk where all named creatures are waiting. Say 'enter' when you are ready.")
