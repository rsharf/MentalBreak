-- Instance Portal for The Skyfire Mountains
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "skyfire"
local ZONE_ID = 91
local LOCKOUT = "inst_skyfire_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = -4286.0, -1140.0, 38.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of The Skyfire Mountains where all named creatures are waiting. Say 'enter' when you are ready.")
