-- The Skyfire Mountains Instance Encounter (version 100)
-- Named NPCs spawn via database spawn2 (version=100)
-- Lockout set on Warder_Cecilia (ID 91037) death

function Boss_Death(e)
    eq.set_global("inst_skyfire_lockout", tostring(os.time()), 2, "H24")
...
function Zone_Entry(e)
    if eq.get_zone_instance_version() == 100 then
        e.self:GMMove(-4286.0, -1140.0, 38.0, 0)
    end
end

function event_encounter_load(e)
    eq.register_npc_event('instance_boss', Event.death_complete, 91037, Boss_Death)
    eq.register_npc_event('instance_boss', Event.combat, 91037, Boss_Combat)
