-- Instance Portal for The Feerrott
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "feerrott"
local ZONE_ID = 47
local LOCKOUT = "inst_feerrott_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 905.0, 1051.0, 25.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of The Feerrott where all named creatures are waiting. Say 'enter' when you are ready.")
