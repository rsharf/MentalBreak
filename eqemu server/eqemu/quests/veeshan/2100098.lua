-- Instance Portal for Veeshan's Peak
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "veeshan"
local ZONE_ID = 108
local LOCKOUT = "inst_veeshan_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 1783.0, -5.0, 15.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Veeshan's Peak where all named creatures are waiting. Say 'enter' when you are ready.")
