-- Instance Portal for Cobaltscar
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "cobaltscar"
local ZONE_ID = 117
local LOCKOUT = "inst_cobaltscar_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 895.0, -939.0, 318.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Cobaltscar where all named creatures are waiting. Say 'enter' when you are ready.")
