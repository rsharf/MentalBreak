-- Instance Portal for Lavastorm Mountains
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "lavastorm"
local ZONE_ID = 27
local LOCKOUT = "inst_lavastorm_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 154.0, -1833.0, -15.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Lavastorm Mountains where all named creatures are waiting. Say 'enter' when you are ready.")
