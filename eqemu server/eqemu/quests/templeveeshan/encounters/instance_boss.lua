-- The Temple of Veeshan Instance Encounter (version 100)
-- Named NPCs spawn via database spawn2 (version=100)
-- Lockout set on Dagarn_the_Destroyer (ID 124011) death

function Boss_Death(e)
    eq.set_global("inst_templeveeshan_lockout", tostring(os.time()), 2, "H24")
...
function Zone_Entry(e)
    if eq.get_zone_instance_version() == 100 then
        e.self:GMMove(-499.0, -2086.0, -36.0, 0)
    end
end

function event_encounter_load(e)
    eq.register_npc_event('instance_boss', Event.death_complete, 124011, Boss_Death)
    eq.register_npc_event('instance_boss', Event.combat, 124011, Boss_Combat)
