-- Instance Portal for The Temple of Veeshan
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "templeveeshan"
local ZONE_ID = 124
local LOCKOUT = "inst_templeveeshan_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = -499.0, -2086.0, -36.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of The Temple of Veeshan where all named creatures are waiting. Say 'enter' when you are ready.")
