-- Instance Portal for The Howling Stones
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "charasis"
local ZONE_ID = 105
local LOCKOUT = "inst_charasis_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 0.0, 0.0, 4.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of The Howling Stones where all named creatures are waiting. Say 'enter' when you are ready.")
