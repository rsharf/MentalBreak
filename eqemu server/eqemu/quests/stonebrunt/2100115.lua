-- Instance Portal for The Stonebrunt Mountains
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "stonebrunt"
local ZONE_ID = 100
local LOCKOUT = "inst_stonebrunt_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = -1643.0, -3428.0, -7.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of The Stonebrunt Mountains where all named creatures are waiting. Say 'enter' when you are ready.")
