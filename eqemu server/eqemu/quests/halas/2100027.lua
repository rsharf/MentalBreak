-- Instance Portal for Halas
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "halas"
local ZONE_ID = 29
local LOCKOUT = "inst_halas_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 0.0, 0.0, 3.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Halas where all named creatures are waiting. Say 'enter' when you are ready.")
