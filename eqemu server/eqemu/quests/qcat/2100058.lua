-- Instance Portal for The Qeynos Aqueduct System
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "qcat"
local ZONE_ID = 45
local LOCKOUT = "inst_qcat_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 80.0, 860.0, -38.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of The Qeynos Aqueduct System where all named creatures are waiting. Say 'enter' when you are ready.")
