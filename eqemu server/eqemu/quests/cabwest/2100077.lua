-- Instance Portal for Cabilis West
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "cabwest"
local ZONE_ID = 82
local LOCKOUT = "inst_cabwest_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 767.0, -783.0, 8.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Cabilis West where all named creatures are waiting. Say 'enter' when you are ready.")
