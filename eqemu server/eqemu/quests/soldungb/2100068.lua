-- Instance Portal for Nagafen's Lair
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "soldungb"
local ZONE_ID = 32
local LOCKOUT = "inst_soldungb_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = -263.0, -424.0, -108.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Nagafen's Lair where all named creatures are waiting. Say 'enter' when you are ready.")
