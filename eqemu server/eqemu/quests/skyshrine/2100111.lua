-- Instance Portal for Skyshrine
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "skyshrine"
local ZONE_ID = 114
local LOCKOUT = "inst_skyshrine_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = -730.0, -210.0, 0.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Skyshrine where all named creatures are waiting. Say 'enter' when you are ready.")
