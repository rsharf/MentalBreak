-- Instance Portal for Southern Desert of Ro
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "sro"
local ZONE_ID = 35
local LOCKOUT = "inst_sro_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 286.0, 1265.0, 79.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Southern Desert of Ro where all named creatures are waiting. Say 'enter' when you are ready.")
