-- Plane of Hate Instance Encounter (version 100)
-- Named NPCs spawn via database spawn2 (version=100)
-- Lockout set on #The_Deathrot_Knight (ID 76057) death

function Boss_Death(e)
    eq.set_global("inst_hateplane_lockout", tostring(os.time()), 2, "H24")
...
function Zone_Entry(e)
    if eq.get_zone_instance_version() == 100 then
        e.self:GMMove(-353.08, -374.8, 3.75, 0)
    end
end

function event_encounter_load(e)
    eq.register_npc_event('instance_boss', Event.death_complete, 76057, Boss_Death)
    eq.register_npc_event('instance_boss', Event.combat, 76057, Boss_Combat)
