-- Instance Portal for Plane of Hate
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "hateplane"
local ZONE_ID = 76
local LOCKOUT = "inst_hateplane_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = -353.08, -374.8, 3.75

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Plane of Hate where all named creatures are waiting. Say 'enter' when you are ready.")
