-- Instance Portal for The Qeynos Hills
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "qeytoqrg"
local ZONE_ID = 4
local LOCKOUT = "inst_qeytoqrg_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 83.0, 508.0, 0.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of The Qeynos Hills where all named creatures are waiting. Say 'enter' when you are ready.")
