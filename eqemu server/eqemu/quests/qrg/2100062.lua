-- Instance Portal for The Surefall Glade
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "qrg"
local ZONE_ID = 3
local LOCKOUT = "inst_qrg_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 0.0, 0.0, 2.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of The Surefall Glade where all named creatures are waiting. Say 'enter' when you are ready.")
