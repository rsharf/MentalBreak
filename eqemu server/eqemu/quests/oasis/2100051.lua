-- Instance Portal for Oasis of Marr
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "oasis"
local ZONE_ID = 37
local LOCKOUT = "inst_oasis_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 903.98, 490.03, 6.4

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Oasis of Marr where all named creatures are waiting. Say 'enter' when you are ready.")
