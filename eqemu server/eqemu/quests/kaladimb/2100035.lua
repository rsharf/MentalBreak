-- Instance Portal for North Kaladim
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "kaladimb"
local ZONE_ID = 67
local LOCKOUT = "inst_kaladimb_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = -267.0, 414.0, 3.75

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of North Kaladim where all named creatures are waiting. Say 'enter' when you are ready.")
