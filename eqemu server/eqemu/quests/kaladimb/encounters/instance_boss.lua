-- North Kaladim Instance Encounter (version 100)
-- Named NPCs spawn via database spawn2 (version=100)
-- Lockout set on Goriba_Thurgorson (ID 67059) death

function Boss_Death(e)
    eq.set_global("inst_kaladimb_lockout", tostring(os.time()), 2, "H24")
...
function Zone_Entry(e)
    if eq.get_zone_instance_version() == 100 then
        e.self:GMMove(-267.0, 414.0, 3.75, 0)
    end
end

function event_encounter_load(e)
    eq.register_npc_event('instance_boss', Event.death_complete, 67059, Boss_Death)
    eq.register_npc_event('instance_boss', Event.combat, 67059, Boss_Combat)
