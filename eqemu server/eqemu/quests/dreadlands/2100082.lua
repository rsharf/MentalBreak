-- Instance Portal for The Dreadlands
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "dreadlands"
local ZONE_ID = 86
local LOCKOUT = "inst_dreadlands_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 9565.0, 2806.0, 1050.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of The Dreadlands where all named creatures are waiting. Say 'enter' when you are ready.")
