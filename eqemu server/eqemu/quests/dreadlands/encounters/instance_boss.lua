-- The Dreadlands Instance Encounter (version 100)
-- Named NPCs spawn via database spawn2 (version=100)
-- Lockout set on Veltar (ID 86092) death

function Boss_Death(e)
    eq.set_global("inst_dreadlands_lockout", tostring(os.time()), 2, "H24")
...
function Zone_Entry(e)
    if eq.get_zone_instance_version() == 100 then
        e.self:GMMove(9565.0, 2806.0, 1050.0, 0)
    end
end

function event_encounter_load(e)
    eq.register_npc_event('instance_boss', Event.death_complete, 86092, Boss_Death)
    eq.register_npc_event('instance_boss', Event.combat, 86092, Boss_Combat)
