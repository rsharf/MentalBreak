-- Instance Portal for Kael Drakkel
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "kael"
local ZONE_ID = 113
local LOCKOUT = "inst_kael_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = -633.0, -47.0, 128.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Kael Drakkel where all named creatures are waiting. Say 'enter' when you are ready.")
