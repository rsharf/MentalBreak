-- Instance Portal for The Southern Plains of Karana
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "southkarana"
local ZONE_ID = 14
local LOCKOUT = "inst_southkarana_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 1294.0, 2348.0, -6.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of The Southern Plains of Karana where all named creatures are waiting. Say 'enter' when you are ready.")
