-- The Southern Plains of Karana Instance Encounter (version 100)
-- Named NPCs spawn via database spawn2 (version=100)
-- Lockout set on #Sir_Telian_Mottsworth (ID 14170) death

function Boss_Death(e)
    eq.set_global("inst_southkarana_lockout", tostring(os.time()), 2, "H24")
...
function Zone_Entry(e)
    if eq.get_zone_instance_version() == 100 then
        e.self:GMMove(1294.0, 2348.0, -6.0, 0)
    end
end

function event_encounter_load(e)
    eq.register_npc_event('instance_boss', Event.death_complete, 14170, Boss_Death)
    eq.register_npc_event('instance_boss', Event.combat, 14170, Boss_Combat)
