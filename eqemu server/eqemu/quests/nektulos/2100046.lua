-- Instance Portal for The Nektulos Forest
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "nektulos"
local ZONE_ID = 25
local LOCKOUT = "inst_nektulos_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = -259.0, -1201.0, -5.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of The Nektulos Forest where all named creatures are waiting. Say 'enter' when you are ready.")
