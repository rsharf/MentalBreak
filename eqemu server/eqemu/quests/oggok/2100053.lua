-- Instance Portal for Oggok
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "oggok"
local ZONE_ID = 49
local LOCKOUT = "inst_oggok_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = -99.0, -345.0, 4.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Oggok where all named creatures are waiting. Say 'enter' when you are ready.")
