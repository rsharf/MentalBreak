-- Oggok Instance Encounter (version 100)
-- Named NPCs spawn via database spawn2 (version=100)
-- Lockout set on Kizrak_the_Tyrant (ID 49074) death

function Boss_Death(e)
    eq.set_global("inst_oggok_lockout", tostring(os.time()), 2, "H24")
...
function Zone_Entry(e)
    if eq.get_zone_instance_version() == 100 then
        e.self:GMMove(-99.0, -345.0, 4.0, 0)
    end
end

function event_encounter_load(e)
    eq.register_npc_event('instance_boss', Event.death_complete, 49074, Boss_Death)
    eq.register_npc_event('instance_boss', Event.combat, 49074, Boss_Combat)
