-- Instance Portal for Everfrost Peaks
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "everfrost"
local ZONE_ID = 30
local LOCKOUT = "inst_everfrost_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 629.0, 3139.0, -60.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Everfrost Peaks where all named creatures are waiting. Say 'enter' when you are ready.")
