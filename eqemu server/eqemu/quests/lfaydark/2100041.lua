-- Instance Portal for The Lesser Faydark
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "lfaydark"
local ZONE_ID = 57
local LOCKOUT = "inst_lfaydark_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = -1770.0, -108.0, 0.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of The Lesser Faydark where all named creatures are waiting. Say 'enter' when you are ready.")
