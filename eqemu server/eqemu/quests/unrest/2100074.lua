-- Instance Portal for The Estate of Unrest
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "unrest"
local ZONE_ID = 63
local LOCKOUT = "inst_unrest_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 52.0, -38.0, 3.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of The Estate of Unrest where all named creatures are waiting. Say 'enter' when you are ready.")
