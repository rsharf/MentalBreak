-- Instance Portal for The Hole
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "hole"
local ZONE_ID = 39
local LOCKOUT = "inst_hole_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = -1050.0, 640.0, -80.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of The Hole where all named creatures are waiting. Say 'enter' when you are ready.")
