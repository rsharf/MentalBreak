-- Instance Portal for Butcherblock Mountains
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "butcher"
local ZONE_ID = 68
local LOCKOUT = "inst_butcher_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = -700.0, 2550.0, 3.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Butcherblock Mountains where all named creatures are waiting. Say 'enter' when you are ready.")
