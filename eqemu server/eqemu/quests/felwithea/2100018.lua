-- Instance Portal for Northern Felwithe
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "felwithea"
local ZONE_ID = 61
local LOCKOUT = "inst_felwithea_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 94.0, -25.0, 3.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Northern Felwithe where all named creatures are waiting. Say 'enter' when you are ready.")
