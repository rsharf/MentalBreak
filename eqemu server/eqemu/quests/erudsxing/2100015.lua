-- Instance Portal for Erud's Crossing
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "erudsxing"
local ZONE_ID = 98
local LOCKOUT = "inst_erudsxing_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 795.0, -1767.0, 11.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Erud's Crossing where all named creatures are waiting. Say 'enter' when you are ready.")
