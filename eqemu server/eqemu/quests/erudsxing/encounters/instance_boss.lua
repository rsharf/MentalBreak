-- Erud's Crossing Instance Encounter (version 100)
-- Named NPCs spawn via database spawn2 (version=100)
-- Lockout set on Yun_Assistant_Maldok (ID 98008) death

function Boss_Death(e)
    eq.set_global("inst_erudsxing_lockout", tostring(os.time()), 2, "H24")
...
function Zone_Entry(e)
    if eq.get_zone_instance_version() == 100 then
        e.self:GMMove(795.0, -1767.0, 11.0, 0)
    end
end

function event_encounter_load(e)
    eq.register_npc_event('instance_boss', Event.death_complete, 98008, Boss_Death)
    eq.register_npc_event('instance_boss', Event.combat, 98008, Boss_Combat)
