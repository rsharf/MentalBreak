-- Instance Portal for East Freeport
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "freporte"
local ZONE_ID = 10
local LOCKOUT = "inst_freporte_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = -648.0, -1097.0, -52.2

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of East Freeport where all named creatures are waiting. Say 'enter' when you are ready.")
