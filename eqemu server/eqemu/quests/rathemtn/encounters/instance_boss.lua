-- The Rathe Mountains Instance Encounter (version 100)
-- Named NPCs spawn via database spawn2 (version=100)
-- Lockout set on Brother_Zephyl (ID 50321) death

function Boss_Death(e)
    eq.set_global("inst_rathemtn_lockout", tostring(os.time()), 2, "H24")
...
function Zone_Entry(e)
    if eq.get_zone_instance_version() == 100 then
        e.self:GMMove(1831.0, 3825.0, 28.0, 0)
    end
end

function event_encounter_load(e)
    eq.register_npc_event('instance_boss', Event.death_complete, 50321, Boss_Death)
    eq.register_npc_event('instance_boss', Event.combat, 50321, Boss_Combat)
