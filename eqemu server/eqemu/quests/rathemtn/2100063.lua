-- Instance Portal for The Rathe Mountains
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "rathemtn"
local ZONE_ID = 50
local LOCKOUT = "inst_rathemtn_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 1831.0, 3825.0, 28.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of The Rathe Mountains where all named creatures are waiting. Say 'enter' when you are ready.")
