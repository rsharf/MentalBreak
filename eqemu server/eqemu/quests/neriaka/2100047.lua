-- Instance Portal for Neriak - Foreign Quarter
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "neriaka"
local ZONE_ID = 40
local LOCKOUT = "inst_neriaka_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 157.0, -3.0, 31.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Neriak - Foreign Quarter where all named creatures are waiting. Say 'enter' when you are ready.")
