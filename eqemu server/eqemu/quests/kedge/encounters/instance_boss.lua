-- Kedge Keep Instance Encounter (version 100)
-- Named NPCs spawn via database spawn2 (version=100)
-- Lockout set on Phinigel_Autropos (ID 64001) death

function Boss_Death(e)
    eq.set_global("inst_kedge_lockout", tostring(os.time()), 2, "H24")
...
function Zone_Entry(e)
    if eq.get_zone_instance_version() == 100 then
        e.self:GMMove(14.0, 100.0, 302.0, 0)
    end
end

function event_encounter_load(e)
    eq.register_npc_event('instance_boss', Event.death_complete, 64001, Boss_Death)
    eq.register_npc_event('instance_boss', Event.combat, 64001, Boss_Combat)
