-- Instance Portal for Kedge Keep
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "kedge"
local ZONE_ID = 64
local LOCKOUT = "inst_kedge_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 14.0, 100.0, 302.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Kedge Keep where all named creatures are waiting. Say 'enter' when you are ready.")
