-- Instance Portal for Erudin
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "erudnext"
local ZONE_ID = 24
local LOCKOUT = "inst_erudnext_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = -338.0, 75.0, 20.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Erudin where all named creatures are waiting. Say 'enter' when you are ready.")
