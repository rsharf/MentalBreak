-- Instance Portal for Karnor's Castle
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "karnor"
local ZONE_ID = 102
local LOCKOUT = "inst_karnor_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 302.0, 18.0, 6.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Karnor's Castle where all named creatures are waiting. Say 'enter' when you are ready.")
