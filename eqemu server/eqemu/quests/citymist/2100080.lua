-- Instance Portal for The City of Mist
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "citymist"
local ZONE_ID = 90
local LOCKOUT = "inst_citymist_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = -734.0, 28.0, 4.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of The City of Mist where all named creatures are waiting. Say 'enter' when you are ready.")
