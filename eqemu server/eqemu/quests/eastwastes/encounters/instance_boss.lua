-- Eastern Wastes Instance Encounter (version 100)
-- Named NPCs spawn via database spawn2 (version=100)
-- Lockout set on A_Legendary_Velious_Dragon (ID 116607) death

function Boss_Death(e)
    eq.set_global("inst_eastwastes_lockout", tostring(os.time()), 2, "H24")
...
function Zone_Entry(e)
    if eq.get_zone_instance_version() == 100 then
        e.self:GMMove(-4296.0, -5049.0, 147.0, 0)
    end
end

function event_encounter_load(e)
    eq.register_npc_event('instance_boss', Event.death_complete, 116607, Boss_Death)
    eq.register_npc_event('instance_boss', Event.combat, 116607, Boss_Combat)
