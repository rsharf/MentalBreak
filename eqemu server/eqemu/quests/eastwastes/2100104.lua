-- Instance Portal for Eastern Wastes
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "eastwastes"
local ZONE_ID = 116
local LOCKOUT = "inst_eastwastes_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = -4296.0, -5049.0, 147.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Eastern Wastes where all named creatures are waiting. Say 'enter' when you are ready.")
