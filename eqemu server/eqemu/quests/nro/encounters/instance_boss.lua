-- Northern Desert of Ro Instance Encounter (version 100)
-- Named NPCs spawn via database spawn2 (version=100)
-- Lockout set on Guard_Brendyl (ID 34042) death

function Boss_Death(e)
    eq.set_global("inst_nro_lockout", tostring(os.time()), 2, "H24")
...
function Zone_Entry(e)
    if eq.get_zone_instance_version() == 100 then
        e.self:GMMove(299.12, 3537.9, -24.5, 0)
    end
end

function event_encounter_load(e)
    eq.register_npc_event('instance_boss', Event.death_complete, 34042, Boss_Death)
    eq.register_npc_event('instance_boss', Event.combat, 34042, Boss_Combat)
