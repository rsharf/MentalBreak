-- Instance Portal for Northern Desert of Ro
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "nro"
local ZONE_ID = 34
local LOCKOUT = "inst_nro_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 299.12, 3537.9, -24.5

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Northern Desert of Ro where all named creatures are waiting. Say 'enter' when you are ready.")
