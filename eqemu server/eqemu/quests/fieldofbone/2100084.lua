-- Instance Portal for The Field of Bone
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "fieldofbone"
local ZONE_ID = 78
local LOCKOUT = "inst_fieldofbone_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 1617.0, -1684.0, -50.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of The Field of Bone where all named creatures are waiting. Say 'enter' when you are ready.")
