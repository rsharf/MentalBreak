-- Instance Portal for Kerra Isle
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "kerraridge"
local ZONE_ID = 74
local LOCKOUT = "inst_kerraridge_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = -859.97, 474.96, 23.75

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Kerra Isle where all named creatures are waiting. Say 'enter' when you are ready.")
