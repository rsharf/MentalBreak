-- Instance Portal for Highpass Hold
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "highpass"
local ZONE_ID = 5
local LOCKOUT = "inst_highpass_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = -104.0, -14.0, 4.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Highpass Hold where all named creatures are waiting. Say 'enter' when you are ready.")
