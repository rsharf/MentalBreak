-- Instance Portal for Neriak - 3rd Gate
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "neriakc"
local ZONE_ID = 42
local LOCKOUT = "inst_neriakc_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = -969.0, 892.0, -52.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Neriak - 3rd Gate where all named creatures are waiting. Say 'enter' when you are ready.")
