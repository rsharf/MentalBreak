-- Instance Portal for The Plane of Mischief
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "mischiefplane"
local ZONE_ID = 126
local LOCKOUT = "inst_mischiefplane_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = -395.0, -1410.0, 115.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of The Plane of Mischief where all named creatures are waiting. Say 'enter' when you are ready.")
