-- Instance Portal for The Warrens
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "warrens"
local ZONE_ID = 101
local LOCKOUT = "inst_warrens_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = -930.0, 748.0, -37.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of The Warrens where all named creatures are waiting. Say 'enter' when you are ready.")
