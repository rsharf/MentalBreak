-- Instance Portal for The Ruins of Sebilis
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "sebilis"
local ZONE_ID = 89
local LOCKOUT = "inst_sebilis_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 0.0, 250.0, 44.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of The Ruins of Sebilis where all named creatures are waiting. Say 'enter' when you are ready.")
