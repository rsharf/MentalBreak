-- Instance Portal for South Qeynos
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "qeynos"
local ZONE_ID = 1
local LOCKOUT = "inst_qeynos_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 0.0, 10.0, 5.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of South Qeynos where all named creatures are waiting. Say 'enter' when you are ready.")
