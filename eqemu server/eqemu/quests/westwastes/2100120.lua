-- Instance Portal for The Western Wastes
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "westwastes"
local ZONE_ID = 120
local LOCKOUT = "inst_westwastes_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = -3499.0, -4099.0, -18.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of The Western Wastes where all named creatures are waiting. Say 'enter' when you are ready.")
