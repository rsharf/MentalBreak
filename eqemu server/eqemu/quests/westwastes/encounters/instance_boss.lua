-- The Western Wastes Instance Encounter (version 100)
-- Named NPCs spawn via database spawn2 (version=100)
-- Lockout set on Sir_Elmonious_Falmont (ID 120133) death

function Boss_Death(e)
    eq.set_global("inst_westwastes_lockout", tostring(os.time()), 2, "H24")
...
function Zone_Entry(e)
    if eq.get_zone_instance_version() == 100 then
        e.self:GMMove(-3499.0, -4099.0, -18.0, 0)
    end
end

function event_encounter_load(e)
    eq.register_npc_event('instance_boss', Event.death_complete, 120133, Boss_Death)
    eq.register_npc_event('instance_boss', Event.combat, 120133, Boss_Combat)
