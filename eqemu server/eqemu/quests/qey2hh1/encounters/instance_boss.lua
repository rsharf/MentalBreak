-- The Western Plains of Karana Instance Encounter (version 100)
-- Named NPCs spawn via database spawn2 (version=100)
-- Lockout set on #Sir_Arlindo_Goldheart (ID 12189) death

function Boss_Death(e)
    eq.set_global("inst_qey2hh1_lockout", tostring(os.time()), 2, "H24")
...
function Zone_Entry(e)
    if eq.get_zone_instance_version() == 100 then
        e.self:GMMove(-531.0, 15.0, -3.0, 0)
    end
end

function event_encounter_load(e)
    eq.register_npc_event('instance_boss', Event.death_complete, 12189, Boss_Death)
    eq.register_npc_event('instance_boss', Event.combat, 12189, Boss_Combat)
