-- Instance Portal for The Western Plains of Karana
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "qey2hh1"
local ZONE_ID = 12
local LOCKOUT = "inst_qey2hh1_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = -531.0, 15.0, -3.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of The Western Plains of Karana where all named creatures are waiting. Say 'enter' when you are ready.")
