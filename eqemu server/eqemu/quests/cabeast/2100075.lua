-- Instance Portal for Cabilis East
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "cabeast"
local ZONE_ID = 106
local LOCKOUT = "inst_cabeast_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = -417.0, 1362.0, 8.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Cabilis East where all named creatures are waiting. Say 'enter' when you are ready.")
