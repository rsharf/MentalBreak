-- Instance Portal for Blackburrow
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "blackburrow"
local ZONE_ID = 17
local LOCKOUT = "inst_blackburrow_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 39.0, -159.0, 3.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Blackburrow where all named creatures are waiting. Say 'enter' when you are ready.")
