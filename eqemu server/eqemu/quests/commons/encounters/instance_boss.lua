-- West Commonlands Instance Encounter (version 100)
-- Named NPCs spawn via database spawn2 (version=100)
-- Lockout set on A_Legendary_Hill_Giant (ID 21163) death

function Boss_Death(e)
    eq.set_global("inst_commons_lockout", tostring(os.time()), 2, "H24")
...
function Zone_Entry(e)
    if eq.get_zone_instance_version() == 100 then
        e.self:GMMove(-1334.24, 209.57, -51.47, 0)
    end
end

function event_encounter_load(e)
    eq.register_npc_event('instance_boss', Event.death_complete, 21163, Boss_Death)
    eq.register_npc_event('instance_boss', Event.combat, 21163, Boss_Combat)
