-- Instance Portal for West Commonlands
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "commons"
local ZONE_ID = 21
local LOCKOUT = "inst_commons_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = -1334.24, 209.57, -51.47

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of West Commonlands where all named creatures are waiting. Say 'enter' when you are ready.")
