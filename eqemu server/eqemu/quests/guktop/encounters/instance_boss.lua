-- The City of Guk Instance Encounter (version 100)
-- Named NPCs spawn via database spawn2 (version=100)
-- Lockout set on an_old_froglok (ID 65089) death

function Boss_Death(e)
    eq.set_global("inst_guktop_lockout", tostring(os.time()), 2, "H24")
...
function Zone_Entry(e)
    if eq.get_zone_instance_version() == 100 then
        e.self:GMMove(7.0, -36.0, 4.0, 0)
    end
end

function event_encounter_load(e)
    eq.register_npc_event('instance_boss', Event.death_complete, 65089, Boss_Death)
    eq.register_npc_event('instance_boss', Event.combat, 65089, Boss_Combat)
