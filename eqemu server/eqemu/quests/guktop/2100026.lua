-- Instance Portal for The City of Guk
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "guktop"
local ZONE_ID = 65
local LOCKOUT = "inst_guktop_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 7.0, -36.0, 4.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of The City of Guk where all named creatures are waiting. Say 'enter' when you are ready.")
