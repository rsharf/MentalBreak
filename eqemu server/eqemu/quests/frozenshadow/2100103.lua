-- Instance Portal for The Tower of Frozen Shadow
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "frozenshadow"
local ZONE_ID = 111
local LOCKOUT = "inst_frozenshadow_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 200.0, 120.0, 0.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of The Tower of Frozen Shadow where all named creatures are waiting. Say 'enter' when you are ready.")
