-- Instance Portal for The Burning Wood
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "burningwood"
local ZONE_ID = 87
local LOCKOUT = "inst_burningwood_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = -821.0, -4942.0, 204.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of The Burning Wood where all named creatures are waiting. Say 'enter' when you are ready.")
