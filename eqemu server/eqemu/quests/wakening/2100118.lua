-- Instance Portal for The Wakening Land
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "wakening"
local ZONE_ID = 119
local LOCKOUT = "inst_wakening_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = -5000.0, -673.0, -195.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of The Wakening Land where all named creatures are waiting. Say 'enter' when you are ready.")
