-- The Crystal Caverns Instance Encounter (version 100)
-- Named NPCs spawn via database spawn2 (version=100)
-- Lockout set on Queen_Dracnia (ID 121072) death

function Boss_Death(e)
    eq.set_global("inst_crystal_lockout", tostring(os.time()), 2, "H24")
...
function Zone_Entry(e)
    if eq.get_zone_instance_version() == 100 then
        e.self:GMMove(303.0, 487.0, -74.0, 0)
    end
end

function event_encounter_load(e)
    eq.register_npc_event('instance_boss', Event.death_complete, 121072, Boss_Death)
    eq.register_npc_event('instance_boss', Event.combat, 121072, Boss_Combat)
