-- Instance Portal for The Crystal Caverns
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "crystal"
local ZONE_ID = 121
local LOCKOUT = "inst_crystal_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 303.0, 487.0, -74.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of The Crystal Caverns where all named creatures are waiting. Say 'enter' when you are ready.")
