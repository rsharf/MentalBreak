-- Instance Portal for The City of Thurgadin
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "thurgadina"
local ZONE_ID = 115
local LOCKOUT = "inst_thurgadina_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 0.0, -1222.0, 0.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of The City of Thurgadin where all named creatures are waiting. Say 'enter' when you are ready.")
