-- Instance Portal for Gorge of King Xorbb
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "beholder"
local ZONE_ID = 16
local LOCKOUT = "inst_beholder_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = -21.44, -512.23, 45.13

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Gorge of King Xorbb where all named creatures are waiting. Say 'enter' when you are ready.")
