-- Gorge of King Xorbb Instance Encounter (version 100)
-- Named NPCs spawn via database spawn2 (version=100)
-- Lockout set on King_Xorbb (ID 16033) death

function Boss_Death(e)
    eq.set_global("inst_beholder_lockout", tostring(os.time()), 2, "H24")
...
function Zone_Entry(e)
    if eq.get_zone_instance_version() == 100 then
        e.self:GMMove(-21.44, -512.23, 45.13, 0)
    end
end

function event_encounter_load(e)
    eq.register_npc_event('instance_boss', Event.death_complete, 16033, Boss_Death)
    eq.register_npc_event('instance_boss', Event.combat, 16033, Boss_Combat)
