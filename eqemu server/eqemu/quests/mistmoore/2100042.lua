-- Instance Portal for The Castle of Mistmoore
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "mistmoore"
local ZONE_ID = 59
local LOCKOUT = "inst_mistmoore_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 120.0, -330.0, -178.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of The Castle of Mistmoore where all named creatures are waiting. Say 'enter' when you are ready.")
