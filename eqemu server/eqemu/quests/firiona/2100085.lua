-- Instance Portal for Firiona Vie
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "firiona"
local ZONE_ID = 84
local LOCKOUT = "inst_firiona_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 1440.0, -2392.0, 1.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Firiona Vie where all named creatures are waiting. Say 'enter' when you are ready.")
