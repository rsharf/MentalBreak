-- Instance Portal for Kaesora
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "kaesora"
local ZONE_ID = 88
local LOCKOUT = "inst_kaesora_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 40.0, 370.0, 102.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Kaesora where all named creatures are waiting. Say 'enter' when you are ready.")
