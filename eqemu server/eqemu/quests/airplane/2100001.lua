-- Instance Portal for The Plane of Sky
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "airplane"
local ZONE_ID = 71
local LOCKOUT = "inst_airplane_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 614.0, 1415.0, -650.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of The Plane of Sky where all named creatures are waiting. Say 'enter' when you are ready.")
