-- The Plane of Sky Instance Encounter (version 100)
-- Named NPCs spawn via database spawn2 (version=100)
-- Lockout set on Eye_of_Veeshan (ID 71065) death

function Boss_Death(e)
    eq.set_global("inst_airplane_lockout", tostring(os.time()), 2, "H24")
...
function Zone_Entry(e)
    if eq.get_zone_instance_version() == 100 then
        e.self:GMMove(614.0, 1415.0, -650.0, 0)
    end
end

function event_encounter_load(e)
    eq.register_npc_event('instance_boss', Event.death_complete, 71065, Boss_Death)
    eq.register_npc_event('instance_boss', Event.combat, 71065, Boss_Combat)
