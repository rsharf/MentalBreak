-- Instance Portal for Icewell Keep
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "thurgadinb"
local ZONE_ID = 129
local LOCKOUT = "inst_thurgadinb_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 0.0, 250.0, 0.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Icewell Keep where all named creatures are waiting. Say 'enter' when you are ready.")
