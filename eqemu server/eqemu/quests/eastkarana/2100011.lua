-- Instance Portal for Eastern Plains of Karana
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "eastkarana"
local ZONE_ID = 15
local LOCKOUT = "inst_eastkarana_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 865.0, 15.0, -33.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Eastern Plains of Karana where all named creatures are waiting. Say 'enter' when you are ready.")
