-- Instance Portal for Steamfont Mountains
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "steamfont"
local ZONE_ID = 56
local LOCKOUT = "inst_steamfont_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = -272.86, 159.86, -21.4

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Steamfont Mountains where all named creatures are waiting. Say 'enter' when you are ready.")
