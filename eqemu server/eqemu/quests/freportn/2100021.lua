-- Instance Portal for North Freeport
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "freportn"
local ZONE_ID = 8
local LOCKOUT = "inst_freportn_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 211.0, -296.0, 4.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of North Freeport where all named creatures are waiting. Say 'enter' when you are ready.")
