-- Instance Portal for The Iceclad Ocean
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "iceclad"
local ZONE_ID = 110
local LOCKOUT = "inst_iceclad_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 340.0, 5330.0, -17.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of The Iceclad Ocean where all named creatures are waiting. Say 'enter' when you are ready.")
