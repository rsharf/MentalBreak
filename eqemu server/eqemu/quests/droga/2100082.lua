-- Instance Portal for Mines of Droga
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "droga"
local ZONE_ID = 81
local LOCKOUT = "inst_droga_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 290.0, 1375.0, 6.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Mines of Droga where all named creatures are waiting. Say 'enter' when you are ready.")
