-- Mines of Droga Instance Encounter (version 100)
-- Named NPCs spawn via database spawn2 (version=100)
-- Lockout set on a_goblin_traitor (ID 81206) death

function Boss_Death(e)
    eq.set_global("inst_droga_lockout", tostring(os.time()), 2, "H24")
...
function Zone_Entry(e)
    if eq.get_zone_instance_version() == 100 then
        e.self:GMMove(290.0, 1375.0, 6.0, 0)
    end
end

function event_encounter_load(e)
    eq.register_npc_event('instance_boss', Event.death_complete, 81206, Boss_Death)
    eq.register_npc_event('instance_boss', Event.combat, 81206, Boss_Combat)
