-- Instance Portal for West Freeport
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "freportw"
local ZONE_ID = 9
local LOCKOUT = "inst_freportw_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 181.0, 335.0, -24.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of West Freeport where all named creatures are waiting. Say 'enter' when you are ready.")
