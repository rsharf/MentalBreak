-- Instance Portal for The Erudin Palace
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "erudnint"
local ZONE_ID = 23
local LOCKOUT = "inst_erudnint_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 808.0, 712.0, 21.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of The Erudin Palace where all named creatures are waiting. Say 'enter' when you are ready.")
