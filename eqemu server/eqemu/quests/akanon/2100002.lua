-- Instance Portal for Ak'Anon
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "akanon"
local ZONE_ID = 55
local LOCKOUT = "inst_akanon_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = -35.0, 47.0, 4.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Ak'Anon where all named creatures are waiting. Say 'enter' when you are ready.")
