-- Instance Portal for The Overthere
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "overthere"
local ZONE_ID = 93
local LOCKOUT = "inst_overthere_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 1450.0, -3500.0, 309.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of The Overthere where all named creatures are waiting. Say 'enter' when you are ready.")
