-- Instance Portal for Neriak - Commons
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "neriakb"
local ZONE_ID = 41
local LOCKOUT = "inst_neriakb_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = -500.0, 3.0, -10.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Neriak - Commons where all named creatures are waiting. Say 'enter' when you are ready.")
