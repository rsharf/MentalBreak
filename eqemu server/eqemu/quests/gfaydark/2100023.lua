-- Instance Portal for The Greater Faydark
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "gfaydark"
local ZONE_ID = 54
local LOCKOUT = "inst_gfaydark_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 10.0, -20.0, 0.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of The Greater Faydark where all named creatures are waiting. Say 'enter' when you are ready.")
