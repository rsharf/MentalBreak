-- Instance Portal for Dragon Necropolis
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "necropolis"
local ZONE_ID = 123
local LOCKOUT = "inst_necropolis_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 2000.0, -100.0, 5.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Dragon Necropolis where all named creatures are waiting. Say 'enter' when you are ready.")
