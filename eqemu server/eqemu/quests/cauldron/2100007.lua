-- Instance Portal for Dagnor's Cauldron
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "cauldron"
local ZONE_ID = 70
local LOCKOUT = "inst_cauldron_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 320.0, 2815.0, 473.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Dagnor's Cauldron where all named creatures are waiting. Say 'enter' when you are ready.")
