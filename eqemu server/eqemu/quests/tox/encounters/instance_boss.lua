-- Toxxulia Forest Instance Encounter (version 100)
-- Named NPCs spawn via database spawn2 (version=100)
-- Lockout set on Sentinel_Flavius (ID 38075) death

function Boss_Death(e)
    eq.set_global("inst_tox_lockout", tostring(os.time()), 2, "H24")
...
function Zone_Entry(e)
    if eq.get_zone_instance_version() == 100 then
        e.self:GMMove(203.0, 2295.0, -45.0, 0)
    end
end

function event_encounter_load(e)
    eq.register_npc_event('instance_boss', Event.death_complete, 38075, Boss_Death)
    eq.register_npc_event('instance_boss', Event.combat, 38075, Boss_Combat)
