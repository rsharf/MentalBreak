-- Instance Portal for Toxxulia Forest
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "tox"
local ZONE_ID = 38
local LOCKOUT = "inst_tox_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 203.0, 2295.0, -45.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Toxxulia Forest where all named creatures are waiting. Say 'enter' when you are ready.")
