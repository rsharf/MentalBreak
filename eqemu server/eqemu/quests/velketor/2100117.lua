-- Instance Portal for Velketor's Labyrinth
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "velketor"
local ZONE_ID = 112
local LOCKOUT = "inst_velketor_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = -65.0, 581.0, -152.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Velketor's Labyrinth where all named creatures are waiting. Say 'enter' when you are ready.")
