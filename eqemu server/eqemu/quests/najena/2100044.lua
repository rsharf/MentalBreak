-- Instance Portal for Najena
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "najena"
local ZONE_ID = 44
local LOCKOUT = "inst_najena_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 858.0, -76.0, 4.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Najena where all named creatures are waiting. Say 'enter' when you are ready.")
