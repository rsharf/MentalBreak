-- Instance Portal for Accursed Temple of CazicThule
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "cazicthule"
local ZONE_ID = 48
local LOCKOUT = "inst_cazicthule_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = -74.0, 71.0, 4.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Accursed Temple of CazicThule where all named creatures are waiting. Say 'enter' when you are ready.")
