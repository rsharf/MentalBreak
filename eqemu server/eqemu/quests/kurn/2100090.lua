-- Instance Portal for Kurn's Tower
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "kurn"
local ZONE_ID = 97
local LOCKOUT = "inst_kurn_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 0.0, 0.0, 7.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Kurn's Tower where all named creatures are waiting. Say 'enter' when you are ready.")
