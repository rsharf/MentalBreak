-- Instance Portal for Ocean of Tears
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "oot"
local ZONE_ID = 69
local LOCKOUT = "inst_oot_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = -9200.0, 390.0, 6.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Ocean of Tears where all named creatures are waiting. Say 'enter' when you are ready.")
