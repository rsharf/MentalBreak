-- Instance Portal for Mines of Nurga
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "nurga"
local ZONE_ID = 107
local LOCKOUT = "inst_nurga_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = -1762.0, -2200.0, 6.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Mines of Nurga where all named creatures are waiting. Say 'enter' when you are ready.")
