-- The Plane of Growth Instance Encounter (version 100)
-- Named NPCs spawn via database spawn2 (version=100)
-- Lockout set on Guardian_of_Takish (ID 127035) death

function Boss_Death(e)
    eq.set_global("inst_growthplane_lockout", tostring(os.time()), 2, "H24")
...
function Zone_Entry(e)
    if eq.get_zone_instance_version() == 100 then
        e.self:GMMove(3016.0, -2522.0, -19.0, 0)
    end
end

function event_encounter_load(e)
    eq.register_npc_event('instance_boss', Event.death_complete, 127035, Boss_Death)
    eq.register_npc_event('instance_boss', Event.combat, 127035, Boss_Combat)
