-- Instance Portal for The Plane of Growth
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "growthplane"
local ZONE_ID = 127
local LOCKOUT = "inst_growthplane_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 3016.0, -2522.0, -19.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of The Plane of Growth where all named creatures are waiting. Say 'enter' when you are ready.")
