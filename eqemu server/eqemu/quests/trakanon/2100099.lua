-- Instance Portal for Trakanon's Teeth
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "trakanon"
local ZONE_ID = 95
local LOCKOUT = "inst_trakanon_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 1486.0, 3868.0, -336.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Trakanon's Teeth where all named creatures are waiting. Say 'enter' when you are ready.")
