-- Instance Portal for Lake of Ill Omen
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "lakeofillomen"
local ZONE_ID = 85
local LOCKOUT = "inst_lakeofillomen_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = -5383.0, 5747.0, 70.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Lake of Ill Omen where all named creatures are waiting. Say 'enter' when you are ready.")
