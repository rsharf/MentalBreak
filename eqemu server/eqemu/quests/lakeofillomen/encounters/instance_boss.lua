-- Lake of Ill Omen Instance Encounter (version 100)
-- Named NPCs spawn via database spawn2 (version=100)
-- Lockout set on Warlord_Geot (ID 85152) death

function Boss_Death(e)
    eq.set_global("inst_lakeofillomen_lockout", tostring(os.time()), 2, "H24")
...
function Zone_Entry(e)
    if eq.get_zone_instance_version() == 100 then
        e.self:GMMove(-5383.0, 5747.0, 70.0, 0)
    end
end

function event_encounter_load(e)
    eq.register_npc_event('instance_boss', Event.death_complete, 85152, Boss_Death)
    eq.register_npc_event('instance_boss', Event.combat, 85152, Boss_Combat)
