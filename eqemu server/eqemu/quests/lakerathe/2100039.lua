-- Instance Portal for Lake Rathetear
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "lakerathe"
local ZONE_ID = 51
local LOCKOUT = "inst_lakerathe_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 1213.0, 4183.0, 3.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Lake Rathetear where all named creatures are waiting. Say 'enter' when you are ready.")
