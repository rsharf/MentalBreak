-- Lake Rathetear Instance Encounter (version 100)
-- Named NPCs spawn via database spawn2 (version=100)
-- Lockout set on Kazen_Fecae (ID 51048) death

function Boss_Death(e)
    eq.set_global("inst_lakerathe_lockout", tostring(os.time()), 2, "H24")
...
function Zone_Entry(e)
    if eq.get_zone_instance_version() == 100 then
        e.self:GMMove(1213.0, 4183.0, 3.0, 0)
    end
end

function event_encounter_load(e)
    eq.register_npc_event('instance_boss', Event.death_complete, 51048, Boss_Death)
    eq.register_npc_event('instance_boss', Event.combat, 51048, Boss_Combat)
