-- The Great Divide Instance Encounter (version 100)
-- Named NPCs spawn via database spawn2 (version=100)
-- Lockout set on #Narandi_the_Wretched (ID 118145) death

function Boss_Death(e)
    eq.set_global("inst_greatdivide_lockout", tostring(os.time()), 2, "H24")
...
function Zone_Entry(e)
    if eq.get_zone_instance_version() == 100 then
        e.self:GMMove(-965.0, -7720.0, -557.0, 0)
    end
end

function event_encounter_load(e)
    eq.register_npc_event('instance_boss', Event.death_complete, 118145, Boss_Death)
    eq.register_npc_event('instance_boss', Event.combat, 118145, Boss_Combat)
