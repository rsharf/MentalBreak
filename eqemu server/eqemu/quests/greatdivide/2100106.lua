-- Instance Portal for The Great Divide
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "greatdivide"
local ZONE_ID = 118
local LOCKOUT = "inst_greatdivide_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = -965.0, -7720.0, -557.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of The Great Divide where all named creatures are waiting. Say 'enter' when you are ready.")
