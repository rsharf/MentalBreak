-- Instance Portal for East Commonlands
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "ecommons"
local ZONE_ID = 22
local LOCKOUT = "inst_ecommons_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = -1485.0, 9.2, -51.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of East Commonlands where all named creatures are waiting. Say 'enter' when you are ready.")
