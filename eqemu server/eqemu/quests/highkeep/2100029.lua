-- Instance Portal for High Keep
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "highkeep"
local ZONE_ID = 6
local LOCKOUT = "inst_highkeep_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 88.0, -16.0, 4.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of High Keep where all named creatures are waiting. Say 'enter' when you are ready.")
