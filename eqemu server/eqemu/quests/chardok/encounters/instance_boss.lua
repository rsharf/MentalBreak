-- Chardok Instance Encounter (version 100)
-- Named NPCs spawn via database spawn2 (version=100)
-- Lockout set on #Historian_Silren_Di`zok (ID 103217) death

function Boss_Death(e)
    eq.set_global("inst_chardok_lockout", tostring(os.time()), 2, "H24")
...
function Zone_Entry(e)
    if eq.get_zone_instance_version() == 100 then
        e.self:GMMove(859.0, 119.0, 106.0, 0)
    end
end

function event_encounter_load(e)
    eq.register_npc_event('instance_boss', Event.death_complete, 103217, Boss_Death)
    eq.register_npc_event('instance_boss', Event.combat, 103217, Boss_Combat)
