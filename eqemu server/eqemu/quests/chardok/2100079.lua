-- Instance Portal for Chardok
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "chardok"
local ZONE_ID = 103
local LOCKOUT = "inst_chardok_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 859.0, 119.0, 106.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of Chardok where all named creatures are waiting. Say 'enter' when you are ready.")
