-- The Jaggedpine Forest Instance Encounter (version 100)
-- Named NPCs spawn via database spawn2 (version=100)
-- Lockout set on #Queen_Nhyalia (ID 181120) death

function Boss_Death(e)
    eq.set_global("inst_jaggedpine_lockout", tostring(os.time()), 2, "H24")
...
function Zone_Entry(e)
    if eq.get_zone_instance_version() == 100 then
        e.self:GMMove(1800.0, 1319.0, -13.0, 0)
    end
end

function event_encounter_load(e)
    eq.register_npc_event('instance_boss', Event.death_complete, 181120, Boss_Death)
    eq.register_npc_event('instance_boss', Event.combat, 181120, Boss_Combat)
