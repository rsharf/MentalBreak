-- Instance Portal for The Jaggedpine Forest
local INST_VERSION = 100
local INST_DURATION = 86400
local ZONE_SHORT = "jaggedpine"
local ZONE_ID = 181
local LOCKOUT = "inst_jaggedpine_lockout"
local SAFE_X, SAFE_Y, SAFE_Z = 1800.0, 1319.0, -13.0

function event_say(e)
    local text = string.lower(e.message)
    if string.find(text, "hail") then
        e.self:Say("I can send you to a private instance of The Jaggedpine Forest where all named creatures are waiting. Say 'enter' when you are ready.")
