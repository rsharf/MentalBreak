...
namespace PetInfoType {
	...
	// Multi-pet DB encoding: even = current, odd = suspended
	// Slot 0: 0/1, Slot 1: 2/3, Slot 2: 4/5
	constexpr int Current0   = 0;
	constexpr int Suspended0 = 1;
	constexpr int Current1   = 2;
	constexpr int Suspended1 = 3;
	constexpr int Current2   = 4;
	constexpr int Suspended2 = 5;

	inline constexpr int PetInfoCurrent(int slot) { return slot * 2; }
	inline constexpr int PetInfoSuspended(int slot) { return slot * 2 + 1; }
	inline constexpr int PetInfoSlot(int type) { return type / 2; }
	inline constexpr bool PetInfoIsSuspended(int type) { return (type % 2) != 0; }
}

namespace MultiPet {
	constexpr int MaxSlots      = 3;
	constexpr int Slot_SummonPet = 0;  // SpellEffect::SummonPet (33) + Familiar (108)
	constexpr int Slot_NecPet    = 1;  // SpellEffect::NecPet (71)
	constexpr int Slot_BSTPet    = 2;  // SpellEffect::SummonBSTPet (106)
}
