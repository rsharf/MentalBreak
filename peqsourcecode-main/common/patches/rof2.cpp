...
#undef PutFieldN
	...
	ENCODE(OP_PlayerProfile)
	{
		...
		*p = nullptr;
		...
		// RoF2 client binary only supports 12 gem slots in profile packet.
		// Extended slots (13-16) are managed via custom DLL opcode.
		constexpr uint32_t kClientGems = 12;
		outapp->WriteUInt32(kClientGems);		// Memorised spell slots

		for (uint32 r = 0; r < kClientGems; r++)
		{
			outapp->WriteUInt32(emu->mem_spells[r]);
		}

		// Keep refresh count at 13 (12 gem timers + 1 HT/LoH) to match client binary expectations
		constexpr uint32_t kRefreshCount = 13;
		outapp->WriteUInt32(kRefreshCount);
...
//			outapp->WriteUInt32(1);
	...
	ENCODE(OP_SpecialMesg)
	{
		...
		*p = nullptr;
		...
		if (old_message.find('\x12') != std::string::npos || new_message.find('\x12') != std::string::npos) {
			LogInfo("[SaylinkDebug] OP_SpecialMesg ENCODE | old_len=[{}] new_len=[{}] old_has_0x12=[{}] new_has_0x12=[{}]",
				old_message.length(), new_message.length(),
				(old_message.find('\x12') != std::string::npos ? "YES" : "NO"),
				(new_message.find('\x12') != std::string::npos ? "YES" : "NO"));
			// Hex dump of new_message (what goes to client)
			std::string hex_dump;
			for (size_t i = 0; i < new_message.length() && i < 200; i++) {
				char hb[4];
				snprintf(hb, 4, "%02X ", (unsigned char)new_message[i]);
				hex_dump += hb;
			}
			LogInfo("[SaylinkDebug] OP_SpecialMesg ENCODE hex to client: {}", hex_dump);
		}
...
// DECODE methods
	...
	DECODE(OP_LoadSpellSet)
	{
		...
		// Client struct is uint8 spell[12] — cap at 12 to prevent overread
		constexpr unsigned int kClientGems = 12;
		for (unsigned int i = 0; i < kClientGems; ++i)
		{
			if (eq->spell[i] == 0)
		...
		// Initialize extended gem slots to empty
		for (unsigned int i = kClientGems; i < EQ::spells::SPELL_GEM_COUNT; ++i)
		{
			emu->spell[i] = 0xFFFFFFFF;
		}
...
// file scope helper methods
	...
	static inline spells::CastingSlot ServerToRoF2CastingSlot(EQ::spells::CastingSlot slot)
	{
		...
		case EQ::spells::CastingSlot::Gem13:
			return spells::CastingSlot::Gem13;
		case EQ::spells::CastingSlot::Gem14:
			return spells::CastingSlot::Gem14;
		case EQ::spells::CastingSlot::Gem15:
			return spells::CastingSlot::Gem15;
		case EQ::spells::CastingSlot::Gem16:
			return spells::CastingSlot::Gem16;
	...
	static inline EQ::spells::CastingSlot RoF2ToServerCastingSlot(spells::CastingSlot slot)
	{
		...
		case spells::CastingSlot::Gem13:
			return EQ::spells::CastingSlot::Gem13;
		case spells::CastingSlot::Gem14:
			return EQ::spells::CastingSlot::Gem14;
		case spells::CastingSlot::Gem15:
			return EQ::spells::CastingSlot::Gem15;
		case spells::CastingSlot::Gem16:
			return EQ::spells::CastingSlot::Gem16;
