...
#define SAYLINK_ITEM_ID 1001

// Quest saylinks use a non-existent item ID so the RoF2 client sends
// OP_ItemLinkClick to the server instead of handling the click locally.
// 0xFFFFF = max 5-hex-digit value, well above any real item ID.
#define SAYLINK_QUEST_ITEM_ID 0xFFFFF

// Hash for item 1001 (Cloth Cap), computed via the hash*31+toupper(c) algorithm
// from GetItemLinkHash() in inventory.cpp. Input string:
// "1001Cloth Cap-1-1-1-1-10 0 0 0 639 200 2 1 1 0 10 2 0"
#define SAYLINK_HASH 0xDE0E008F
