...
// Can item be equipped by any class in a multiclass bitmask?
bool EQ::ItemInstance::IsClassEquipableByMask(uint32 class_mask) const
{
	if (!m_item || !m_item->Slots) {
		return false;
	}

	return m_item->IsClassEquipableByMask(class_mask);
}
