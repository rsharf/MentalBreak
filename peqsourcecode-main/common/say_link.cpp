...
const std::string &EQ::SayLinkEngine::GenerateLink()
{
	...
	if ((m_LinkBody.length() == EQ::constants::SAY_LINK_BODY_SIZE) && (m_LinkText.length() > 0)) {
		...
		LogInfo("[SaylinkDebug] GenerateLink OK | body_len=[{}] text=[{}] body=[{}] total_link_len=[{}]",
			m_LinkBody.length(), m_LinkText, m_LinkBody, m_Link.length());
	}
	else {
		LogInfo("[SaylinkDebug] GenerateLink SKIP | body_len=[{}] expected=[{}] text_len=[{}]",
			m_LinkBody.length(), EQ::constants::SAY_LINK_BODY_SIZE, m_LinkText.length());
	}
...
void EQ::SayLinkEngine::generate_body()
{
	...
	*/
	...
	if (m_TaskUse) {
		m_LinkBodyStruct.hash = SAYLINK_HASH;
	}
...
std::string EQ::SayLinkEngine::GenerateQuestSaylink(const std::string& saylink_text, bool silent, const std::string& link_name)
{
	 ...
	 * Uses SAYLINK_ITEM_ID (1001, Cloth Cap) so the RoF2 client can find
	 * the item in its local DB and render the link as colored/clickable.
	 * OP_ItemLinkClick handler intercepts clicks before item inspect runs.
	 */
	EQ::SayLinkEngine linker;
	linker.SetProxyItemID(SAYLINK_ITEM_ID);
	linker.SetProxyHash(SAYLINK_HASH);
	if (silent) {
	...
	std::string result = linker.GenerateLink();
	LogInfo("[SaylinkDebug] GenerateQuestSaylink | phrase=[{}] display=[{}] silent=[{}] saylink_id=[{}] item_id=[{}] result_len=[{}]",
		saylink_text, link_name, silent ? 1 : 0, saylink_id, SAYLINK_ITEM_ID, result.length());
...
std::string EQ::SayLinkEngine::InjectSaylinksIfNotExist(const char *message)
{
	...
	for (; *ch != '\0'; ++ch)
	{
		...
		bool bracket_consumed = false;

		if (in_bracket_state && (*ch == '[' || *ch == '\x12' || abort_space))
		{
			// abort due to nested bracket or existing saylink — restore the suppressed '['
		...
		else if (in_bracket_state && *ch == ']')
		{
			if (ch != startpos)
			{
				...
				// empty brackets [] — restore them as-is
				new_message.push_back('[');
				new_message.push_back(']');
			}
			in_bracket_state = false;
			bracket_consumed = true;  // don't output the ']'
		}

		if (!in_bracket_state && !bracket_consumed)
		{
			if (*ch == '[' && !in_link_state)
			{
				// suppress '[' — it will be restored on abort or consumed by saylink
				startpos = ch + 1;
	...
	// handle unclosed bracket at end of string
	if (in_bracket_state) {
		new_message.push_back('[');
		new_message.append(startpos, ch - startpos);
	}
