...
void Corpse::LootCorpseItem(Client *c, const EQApplicationPacket *app)
{
	...
	if (c && inst) {
		...
		/* Send loot message with plain-text item name.
		 * Note: SayLinkEngine item links inside OP_FormattedMessage %1 args
		 * are mangled by the RoF2 client (truncated names, broken links).
		 * The clickable item link is sent separately via SendItemLink() below.
		 */
		const char* item_name = inst->GetItem()->Name;

		c->MessageString(Chat::Loot, LOOTED_MESSAGE, item_name);
