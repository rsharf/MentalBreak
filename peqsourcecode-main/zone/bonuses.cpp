...
void Client::CalcBonuses()
{
	...
	int timer = HasAnyClass(Class::Monk) ? CONSUMPTION_MNK_TIMER : CONSUMPTION_TIMER;
	timer = timer * (100 + metabolism) / 100;
	if (timer != consume_food_timer.GetTimerTime())
		consume_food_timer.SetTimer(timer);

	// Push updated AC/ATK/stats/resists/weight to client DLL
	if (Connected())
		SendEdgeStatPacket();
}
...
void Mob::AddItemBonuses(const EQ::ItemInstance* inst, StatBonuses* b, bool is_augment, bool is_tribute, int recommended_level_override, bool is_ammo_item) {
	...
	if (IsClient() && !is_tribute && (!inst->IsRaceEquipable(GetBaseRace()) || !inst->IsClassEquipableByMask(CastToClient()->GetClassesMask()))) {
...
void Mob::AdditiveWornBonuses(const EQ::ItemInstance* inst, StatBonuses* b, bool is_augment) {
	...
	*/
	...
	if (IsClient()) {
		if (!inst->IsRaceEquipable(GetBaseRace()) || !inst->IsClassEquipableByMask(CastToClient()->GetClassesMask())) {
			return;
		}
	} else if (!inst->IsEquipable(GetBaseRace(), GetClass())) {
		return;
	}
...
void Mob::CalcSpellBonuses(StatBonuses* newbon)
{
	...
	if (HasAnyClass(Class::Bard) && !IsWisdomCasterClass() && !IsIntelligenceCasterClass())
		newbon->ManaRegen = 0; // Bards do not get mana regen from spells (unless they also have a caster class).
}
...
{
	n->heroic_max_mana += IsWisdomCasterClass() ? GetHeroicWIS() * RuleR(Character, HeroicWisdomMultiplier) * 10 : 0;
	n->heroic_mana_regen += IsWisdomCasterClass() ? GetHeroicWIS() * RuleR(Character, HeroicWisdomMultiplier) / 25 : 0;
	...
	if (RuleB(Character, HeroicStatsUseDataBucketsToScale)) {
		n->heroic_max_mana += IsWisdomCasterClass() ? GetHeroicWIS() * CheckHeroicBonusesDataBuckets(HeroicBonusBucket::WisMaxMana) * 10 : 0;
		n->heroic_mana_regen += IsWisdomCasterClass() ? GetHeroicWIS() * CheckHeroicBonusesDataBuckets(HeroicBonusBucket::WisManaRegen) / 25 : 0;
...
{
	n->heroic_max_mana += IsIntelligenceCasterClass() ? GetHeroicINT() * RuleR(Character, HeroicIntelligenceMultiplier) * 10 : 0;
	n->heroic_mana_regen += IsIntelligenceCasterClass() ? GetHeroicINT() * RuleR(Character, HeroicIntelligenceMultiplier) / 25 : 0;
	n->SpellDmg += GetHeroicINT() * RuleR(Character, HeroicIntelligenceIncreaseSpellDmgMultiplier);

	if (RuleB(Character, HeroicStatsUseDataBucketsToScale)) {
		n->heroic_max_mana += IsIntelligenceCasterClass() ? GetHeroicINT() * CheckHeroicBonusesDataBuckets(HeroicBonusBucket::IntMaxMana) * 10 : 0;
		n->heroic_mana_regen += IsIntelligenceCasterClass() ? GetHeroicINT() * CheckHeroicBonusesDataBuckets(HeroicBonusBucket::IntManaRegen) / 25 : 0;
