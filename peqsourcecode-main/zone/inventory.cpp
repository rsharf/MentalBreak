...
{
	// #1: Try to auto equip
	if (try_worn && inst.IsRaceEquipable(GetBaseRace()) && inst.IsClassEquipableByMask(GetClassesMask()) && inst.GetItem()->ReqLevel <= level && (!inst.GetItem()->Attuneable || inst.IsAttuned()) && inst.GetItem()->ItemType != EQ::item::ItemTypeAugmentation) {
