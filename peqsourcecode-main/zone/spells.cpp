...
bool Client::CheckFizzle(uint16 spell_id)
{
	...
	// Multiclass: find the best (lowest) spell level across all player classes
	// instead of only checking primary class via GetClass()-1
	uint8 mc_spell_level = UINT8_MAX;
	ForEachClass([&](uint8 c) {
		if (c >= 1 && c <= Class::PLAYER_CLASS_COUNT) {
			uint8 lvl = spells[spell_id].classes[c - 1];
			if (lvl < mc_spell_level) {
				mc_spell_level = lvl;
			}
		}
	});

	// Multiclass: determine if any class is INT or WIS caster for stat reduction
	bool mc_has_int_caster = false, mc_has_wis_caster = false, mc_has_bard = false;
	ForEachClass([&](uint8 c) {
		switch (c) {
			case Class::ShadowKnight: case Class::Necromancer:
			case Class::Wizard: case Class::Magician: case Class::Enchanter:
				mc_has_int_caster = true; break;
			case Class::Bard:
				mc_has_bard = true; mc_has_int_caster = true; break;
			case Class::Cleric: case Class::Paladin: case Class::Ranger:
			case Class::Druid: case Class::Shaman: case Class::Beastlord:
				mc_has_wis_caster = true; break;
		}
	});
	...
	if (mc_spell_level < no_fizzle_level) {
		return true;
	}
	...
	// Multiclass: use best spell level across all classes instead of primary only
	par_skill = mc_spell_level * 5 - 10;
	if (par_skill > 235) {
		par_skill = 235;
	}

	par_skill += mc_spell_level; // maximum of 270 for level 65 spell
	...
	// if you have high int/wis you fizzle less, you fizzle more if you are stupid (multiclass-aware)
	if (mc_has_bard) {
							...
							int32  mana_used, uint32 inventory_slot, int16 resist_adjust)
{
	...
	*/
	...
	// a spell bar slot
	if(HasAnyClass(Class::Bard)) // bard's can move when casting any spell...
	{
		if (IsBardSong(spell_id) && slot < CastingSlot::MaxGems) {
			// Songs are permanent fire-and-forget buffs — no pulse/twist needed.
			// CalcBuffDuration returns PERMANENT_BUFF_DURATION for beneficial songs.
			LogSpells("Bard song [{}] casting as permanent buff: slot [{}]", spell_id, (int)slot);
			bard_song_mode = true;
		}
	}
	...
	if(IsClient()) {
		...
		if (item && item->GetItem() && item->GetItem()->CastTime == 0) {
			...
			for(int t_count = 0; t_count < 4; t_count++) {
				...
				if(bard_song_mode) {
					// Instruments not required — all songs can be played freely
					continue;
	...
	if(bard_song_mode)
	{
		...
		{
			Client *c = CastToClient();
			// Re-enable spell bar so the player can cast again (fire-and-forget songs)
			SendSpellBarEnable(spell_id);
			...
			// Tell client casting is finished and mana is available
			SetMana(GetMana());
...
// single target spells
		...
		case ST_SummonedPet:
		{
			if(!spell_target || (spell_target->GetOwnerID() != GetID()) ||
		...
		case ST_Pet:
		{
			...
			if (!bot_casting_on_other_pet) {
				// If current target is one of our pets, keep it; otherwise default to slot 0
				if (spell_target && spell_target->GetOwnerID() == GetID()) {
					// already targeting one of our pets — use it
				} else {
					spell_target = GetPet();
				}
			}
		...
		case ST_GroupClientAndPet:
		{
			...
			{
				if(spell_target->GetOwnerID() == GetID())
				{
					CastAction = SingleTarget;
				}
						...
						uint32 timer, uint32 timer_duration, bool from_casted_spell, uint32 aa_id)
{
			...
			if (spells[spell_id].can_mgb && HasMGB()) {
				...
				if (spell_target->IsGrouped()) {
					...
					if (target_group) {
						...
						if (target_group != GetGroup() && !HasAnyClass(Class::Bard)) {
							SpellOnTarget(spell_id, this);
						}
					}
					...
					if (spell_target != this) {
						...
						//pets too — all slots
						if (spells[spell_id].target_type != ST_GroupNoPets && HasPetAffinity()) {
							for (int ps = 0; ps < 3; ps++) {
								Mob* p = GetPetInSlot(ps);
								if (p && !p->IsCharmed())
									SpellOnTarget(spell_id, p);
							}
						}
	#endif
					}
	...
	#ifdef GROUP_BUFF_PETS
					//pets too — all slots
					if (spells[spell_id].target_type != ST_GroupNoPets && spell_target->HasPetAffinity()) {
						for (int ps = 0; ps < 3; ps++) {
							Mob* p = spell_target->GetPetInSlot(ps);
							if (p && !p->IsCharmed())
								SpellOnTarget(spell_id, p);
						}
					}
	...
	*/
	...
	if(IsClient() && !isproc && !IsFromTriggeredSpell(slot, inventory_slot))
	{
		...
		if (HasAnyClass(Class::Bard) && spell_id != casting_spell_id && timer != 0xFFFFFFFF) {
...
int Mob::CalcBuffDuration(Mob *caster, Mob *target, uint16 spell_id, int32 caster_level_override)
{
	...
	// Bard songs are permanent fire-and-forget buffs — stay until clicked off.
	// Only beneficial songs (short_buff_box >= 0); damage songs (short_buff_box == -1) stay instant.
	if (caster && caster->IsClient() && IsBardSong(spell_id) && spells[spell_id].short_buff_box >= 0) {
	...
	for (buffslot = 0; buffslot < buff_count; buffslot++) {
		...
		if (IsValidSpell(curbuf.spellid)) {
			...
			if (ret == -1) { // stop the spell
				...
				if (caster) {
					if (caster->IsClient() && RuleB(Client, UseLiveBlockedMessage) && !caster->HasAnyClass(Class::Bard)) {
						caster->Message(
							Chat::Red,
							fmt::format(
					...
					if (caster->IsBot() && RuleB(Bots, BotsUseLiveBlockedMessage) && !caster->HasAnyClass(Class::Bard)) {
...
std::unordered_map<uint32, std::vector<uint16>> Client::LoadSpellGroupCache(uint8 min_level, uint8 max_level) {
	...
	// Multiclass: build WHERE clause checking ALL classes, not just primary
	std::string class_conditions;
	ForEachClass([&](uint8 c) {
		if (!c) return;
		if (!class_conditions.empty()) class_conditions += " OR ";
		class_conditions += fmt::format("classes{} BETWEEN {} AND {}", c, min_level, max_level);
	});

	if (class_conditions.empty()) return spell_group_cache;
...
bool Mob::AddProcToWeapon(uint16 spell_id, bool bPerma, uint16 iChance, uint16 base_spell_id, int level_override, uint32 proc_reuse_time) {
	...
	if (spell_id == SPELL_VAMPIRIC_EMBRACE && HasAnyClass(Class::ShadowKnight)) {
...
bool Mob::CheckItemRaceClassDietyRestrictionsOnCast(uint32 inventory_slot) {
	...
	// Multiclass: check ALL classes via bitmask
	uint32 bitmask = CastToClient()->GetClassesMask();
