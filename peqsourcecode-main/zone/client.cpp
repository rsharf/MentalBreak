				   ...
				   bandolier_throttle_timer(0)
{
	...
	// Reserve first 3 XTarget slots for multi-pet health monitoring
	XTargets[0].Type = MyPet;
	XTargets[1].Type = MyPet;
	XTargets[2].Type = MyPet;
	MaxXTargets = 8;
...
{
    if (!m_multiclass_loaded) {
        LogInfo("[Multiclass] PersistMultiClassToDB skipped for CharID {} — multiclass state not yet loaded from DB", CharacterID());
        return;
    }
...
// ---------------------------------------------------------------------------
// Multiclass-aware overrides — check ALL classes, not just primary
// ---------------------------------------------------------------------------
bool Client::IsWarriorClass() const
{
	bool r = false;
	ForEachClass([&](uint8 c) { if (::IsFighterClass(c)) r = true; });
	return r;
}

bool Client::IsIntelligenceCasterClass() const
{
	bool r = false;
	ForEachClass([&](uint8 c) { if (::IsINTCasterClass(c) || ::IsHeroicINTCasterClass(c)) r = true; });
	return r;
}

bool Client::IsWisdomCasterClass() const
{
	bool r = false;
	ForEachClass([&](uint8 c) { if (::IsWISCasterClass(c) || ::IsHeroicWISCasterClass(c)) r = true; });
	return r;
}

bool Client::IsPureMeleeClass() const
{
	// "Pure melee" means ALL classes are pure-melee (no caster/hybrid).
	// If ANY class is a caster or hybrid, the character is NOT pure melee.
	bool all_pure_melee = true;
	ForEachClass([&](uint8 c) {
		switch (c) {
			case Class::Warrior: case Class::Monk:
			case Class::Rogue:   case Class::Berserker:
				break; // still pure melee
			default:
				all_pure_melee = false;
		}
	});
	return all_pure_melee;
}

uint8 Client::GetArchetype() const
{
	// Return best archetype across all classes
	uint8 best = 0;
	ForEachClass([&](uint8 c) {
		uint8 a = 0;
		switch (c) {
			case Class::Paladin: case Class::Ranger:
			case Class::ShadowKnight: case Class::Bard:
			case Class::Beastlord:
				a = Archetype::Hybrid; break;
			case Class::Cleric: case Class::Druid: case Class::Shaman:
			case Class::Wizard: case Class::Magician: case Class::Necromancer:
			case Class::Enchanter:
				a = Archetype::Caster; break;
			default:
				a = Archetype::Melee; break;
		}
		if (a > best) best = a;
	});
	return best;
...
static uint32 ClassLevelFactorForClass(uint8 cls, uint8 mlevel)
{
	switch (cls) {
		case Class::Warrior:
			if (mlevel < 20) return 220;
			else if (mlevel < 30) return 230;
			else if (mlevel < 40) return 250;
			else if (mlevel < 53) return 270;
			else if (mlevel < 57) return 280;
			else if (mlevel < 60) return 290;
			else if (mlevel < 70) return 300;
			else return 311;
		case Class::Druid: case Class::Cleric: case Class::Shaman:
			return (mlevel < 70) ? 150 : 157;
		case Class::Berserker: case Class::Paladin: case Class::ShadowKnight:
			if (mlevel < 35) return 210;
			else if (mlevel < 45) return 220;
			else if (mlevel < 51) return 230;
			else if (mlevel < 56) return 240;
			else if (mlevel < 60) return 250;
			else if (mlevel < 68) return 260;
			else return 270;
		case Class::Monk: case Class::Bard: case Class::Rogue: case Class::Beastlord:
			if (mlevel < 51) return 180;
			else if (mlevel < 58) return 190;
			else if (mlevel < 70) return 200;
			else return 210;
		case Class::Ranger:
			if (mlevel < 58) return 200;
			else if (mlevel < 70) return 210;
			else return 220;
		case Class::Magician: case Class::Wizard: case Class::Necromancer: case Class::Enchanter:
			return (mlevel < 70) ? 120 : 127;
		default:
			if (mlevel < 35) return 210;
			else if (mlevel < 45) return 220;
			else if (mlevel < 51) return 230;
...
uint32 Client::GetClassLevelFactor()
{
	// Return the HIGHEST HP multiplier from all classes (best HP pool)
	uint32 best = 0;
	uint8 mlevel = GetLevel();
	ForEachClass([&](uint8 c) {
		uint32 val = ClassLevelFactorForClass(c, mlevel);
		if (val > best) best = val;
	});
	return best;
}

bool Client::CanThisClassTripleAttack() const
{
	if (RuleB(Combat, ClassicTripleAttack)) {
		bool can = false;
		ForEachClass([&](uint8 c) {
			if (GetLevel() >= 60 && (c == Class::Warrior || c == Class::Ranger ||
				c == Class::Monk || c == Class::Berserker))
				can = true;
		});
		return can;
	}
	return HasSkill(EQ::skills::SkillTripleAttack);
}

bool Client::CanThisClassDualWield(void) const
{
	if (HasSkill(EQ::skills::SkillDualWield)) {
		const EQ::ItemInstance* pinst = GetInv().GetItem(EQ::invslot::slotPrimary);
		const EQ::ItemInstance* sinst = GetInv().GetItem(EQ::invslot::slotSecondary);
		...
		// Dual-Wielding Empty Fists — allow if ANY class is Monk or Beastlord
		if (!pinst && !sinst) {
...
void Client::HydrateMulticlassFromDB()
{
    ...
    // Grant skills from all multiclass classes (Meditation, Alteration, etc.)
    MaxSkills();

    // Recalculate stats so mana pool reflects the new class skills
    CalcBonuses();

    m_multiclass_loaded = true;
}

int Client::GetMulticlassCount() const
{
    int count = 0;
...
int Client::GrantMulticlass(uint8 class_id)
{
    ...
    if (HasClass(class_id)) {
        return 2; // already has class
    }

    // Check if at max (3 classes)
    int current_count = GetMulticlassCount();
    if (current_count >= 3) {
        return 3; // already at 3 classes
    }

    auto charid = CharacterID();
    auto primary = GetClass();

    // Ensure primary row exists in character_classes
    auto q = fmt::format(
        "INSERT IGNORE INTO character_classes (char_id, class_id, is_primary) VALUES ({}, {}, 1)",
        charid, primary
    );
    database.QueryDatabase(q);

    // Insert the new secondary class
    q = fmt::format(
        "INSERT IGNORE INTO character_classes (char_id, class_id, is_primary) VALUES ({}, {}, 0)",
        charid, class_id
    );
    auto r = database.QueryDatabase(q);
    if (!r.Success()) {
        LogError("[Multiclass] GrantMulticlass DB insert failed for char {} class {}: {}",
            charid, class_id, r.ErrorMessage().c_str());
        return 4; // DB error
    }

    // Set quest globals so Lua event_level_up can read multiclass data
    const char* slot_name = (current_count == 1) ? "class2" : "class3";
    q = fmt::format(
        "REPLACE INTO quest_globals (charid, npcid, zoneid, name, value) "
        "VALUES ({}, 0, 0, '{}', '{}')",
        charid, slot_name, class_id
    );
    database.QueryDatabase(q);

    // Hydrate runtime state from DB (updates mask, m_classes array, skills, bonuses)
    HydrateMulticlassFromDB();

    // Send updated multiclass mask to client DLL
    SendMulticlassInfo();

    // Scribe all spells and disciplines for the new level
    ScribeSpells(1, GetLevel());
    LearnDisciplines(1, GetLevel());

    // Resend AA tables so the client receives AAs for the newly added class
    ...
    LogInfo("[Multiclass] GrantMulticlass: char {} granted class {} (count now {})",
        charid, class_id, GetMulticlassCount());
...
bool Client::Save(uint8 iCommitNow) {
	...
	// Snapshot all active pets into their respective PetInfo current slots
	for (int ps = 0; ps < 3; ps++) {
		int cur_type = PetInfoType::PetInfoCurrent(ps);
		Mob* slotpet = GetPetInSlot(ps);
		if (slotpet && slotpet->IsNPC() &&
			slotpet->CastToNPC()->GetPetSpellID() &&
			(!dead || allow_save_while_dead)) {

			NPC* pet = slotpet->CastToNPC();
			m_petinfos[cur_type].SpellID  = pet->GetPetSpellID();
			m_petinfos[cur_type].HP       = pet->GetHP();
			m_petinfos[cur_type].Mana     = pet->GetMana();
			pet->GetPetState(m_petinfos[cur_type].Buffs, m_petinfos[cur_type].Items, m_petinfos[cur_type].Name);
			m_petinfos[cur_type].petpower = pet->GetPetPower();
			m_petinfos[cur_type].size     = pet->GetSize();
			m_petinfos[cur_type].taunting = pet->IsTaunting();
		} else {
			if (!slotpet || !slotpet->IsNPC() || !slotpet->CastToNPC()->GetPetSpellID()) {
				memset(&m_petinfos[cur_type], 0, sizeof(struct PetInfo));
			}
		}
	}
...
void Client::MaxSkills()
{
    for (const auto &s : EQ::skills::GetSkillTypeMap()) {
        ...
        if (current_skill_value == 0 && GetSkill(s.first) > 0 && !EQ::skills::IsSpecializedSkill(s.first)) {
            // Clear stale skills from classes the character no longer has
            SetSkill(s.first, 0);
        }
        else if (GetSkill(s.first) < current_skill_value) {
			...
			if (!bindmob) {
				...
				if (!GetFeigned() && (DistanceSquared(bindmob->GetPosition(), m_Position) <= 400)) {
					...
					if (RuleB(Character, UseOldBindWound)) {
						...
						if (HasAnyClass(Class::Monk) && GetSkill(EQ::skills::SkillBindWound) > 200) {
						...
						if (GetRawSkill(EQ::skills::SkillBindWound) > 200) {
							if ((HasAnyClass(Class::Monk)) || (HasAnyClass(Class::Beastlord)))
								percent_base = 70;
							else if ((GetLevel() > 50) && ((HasAnyClass(Class::Warrior)) || (HasAnyClass(Class::Rogue)) || (HasAnyClass(Class::Cleric))))
								percent_base = 70;
						}
...
void Client::UpdateRestTimer(uint32 new_timer)
{
	...
	bool Client::HasSuspendedMinion() const
	{
		...
		if (is_valid_spell(m_petinfos[PetInfoType::Suspended].SpellID))
			return true;

		// Fallback: DB-loaded petinfo (when we have no live pet)
		if (!HasPet() && is_valid_spell(m_petinfos[PetInfoType::Current].SpellID))
			return true;
	...
	void Client::UnsuspendMinion()
{
    ...
    if (is_valid_spell(m_petinfos[PetInfoType::Suspended].SpellID)) {
        src = &m_petinfos[PetInfoType::Suspended];
        used_suspendedminion = true;
    } else if (is_valid_spell(m_petinfos[PetInfoType::Current].SpellID)) {
        src = &m_petinfos[PetInfoType::Current];
    } else {
    ...
    if (used_suspendedminion) {
        memset(&m_petinfos[PetInfoType::Suspended], 0, sizeof(PetInfo));
    } else {
        memset(&m_petinfos[PetInfoType::Current], 0, sizeof(PetInfo));
    }
    database.SavePetInfo(this);
...
void Client::SuspendMinion(int value)
{
	...
	*/
	...
	if(!CurrentPet)
	{
		if(m_petinfos[PetInfoType::Suspended].SpellID > 0)
		{
			if (m_petinfos[PetInfoType::Suspended].SpellID >= SPDAT_RECORDS) {
				Message(Chat::Red, "Invalid suspended minion spell id (%u).", m_petinfos[PetInfoType::Suspended].SpellID);
				memset(&m_petinfos[PetInfoType::Suspended], 0, sizeof(PetInfo));
				return;
			}

			MakePoweredPet(m_petinfos[PetInfoType::Suspended].SpellID, spells[m_petinfos[PetInfoType::Suspended].SpellID].teleport_zone,
				m_petinfos[PetInfoType::Suspended].petpower, m_petinfos[PetInfoType::Suspended].Name, m_petinfos[PetInfoType::Suspended].size);
			...
			if(value >= 1)
			{
				CurrentPet->SetPetState(m_petinfos[PetInfoType::Suspended].Buffs, m_petinfos[PetInfoType::Suspended].Items);

				CurrentPet->SendPetBuffsToClient();
			}
			CurrentPet->CalcBonuses();

			CurrentPet->SetHP(m_petinfos[PetInfoType::Suspended].HP);

			CurrentPet->SetMana(m_petinfos[PetInfoType::Suspended].Mana);

			CurrentPet->SetTaunting(m_petinfos[PetInfoType::Suspended].taunting);

			MessageString(Chat::Magenta, SUSPEND_MINION_UNSUSPEND, CurrentPet->GetCleanName());

			memset(&m_petinfos[PetInfoType::Suspended], 0, sizeof(struct PetInfo));
	...
	else
	{
		...
		if(SpellID)
		{
			...
			{
				m_petinfos[PetInfoType::Suspended].SpellID = SpellID;

				m_petinfos[PetInfoType::Suspended].HP = CurrentPet->GetHP();

				m_petinfos[PetInfoType::Suspended].Mana = CurrentPet->GetMana();
				m_petinfos[PetInfoType::Suspended].petpower = CurrentPet->GetPetPower();
				m_petinfos[PetInfoType::Suspended].size = CurrentPet->GetSize();

				if(value >= 1)
					CurrentPet->GetPetState(m_petinfos[PetInfoType::Suspended].Buffs, m_petinfos[PetInfoType::Suspended].Items, m_petinfos[PetInfoType::Suspended].Name);
				else
					strn0cpy(m_petinfos[PetInfoType::Suspended].Name, CurrentPet->GetName(), 64); // Name stays even at rank 1
...
void Client::UpdatePetXTarget(int pet_slot, Mob *pet)
{
	if (!XTargettingAvailable() || pet_slot < 0 || pet_slot >= 3)
		return;

	if (pet) {
		XTargets[pet_slot].ID = pet->GetID();
		strn0cpy(XTargets[pet_slot].Name, pet->GetCleanName(), 64);
	} else {
		XTargets[pet_slot].ID = 0;
		XTargets[pet_slot].Name[0] = 0;
	}
	XTargets[pet_slot].Type = MyPet;
	SendXTargetPacket(pet_slot, pet);
}
...
void Client::InitInnates()
{
	...
	// Multiclass: grant innate skills for ALL player classes
	ForEachClass([&](uint8 c) {
		switch (c) {
			case Class::Druid:
...
std::vector<int> Client::GetLearnableDisciplines(uint8 min_level, uint8 max_level) {
	...
	for (uint16 spell_id = 0; spell_id < SPDAT_RECORDS; ++spell_id) {
		...
		// Multiclass: find the best (lowest) discipline level across ALL classes
		uint8 best_disc_level = 255;
		ForEachClass([&](uint8 c) {
			if (!c) return;
			uint8 disc_level = spells[spell_id].classes[c - 1];
			if (disc_level > 0 && disc_level < 255 && disc_level < best_disc_level) {
				best_disc_level = disc_level;
			}
		});

		if (best_disc_level == 255) {
			continue; // No class can use this discipline
		}

		if (max_level && best_disc_level > max_level) {
			continue;
		}

		if (min_level > 1 && best_disc_level < min_level) {
			continue;
		}
...
std::vector<int> Client::GetScribeableSpells(uint8 min_level, uint8 max_level) {
	...
	for (uint16 spell_id = 0; spell_id < SPDAT_RECORDS; ++spell_id) {
		...
		// Multiclass: find the best (lowest) spell level across ALL classes
		uint8 best_spell_level = 255;
		ForEachClass([&](uint8 c) {
			if (!c) return;
			uint8 spell_level = spells[spell_id].classes[c - 1];
			if (spell_level > 0 && spell_level < 255 && spell_level < best_spell_level) {
				best_spell_level = spell_level;
			}
		});

		if (best_spell_level == 255) {
			continue; // No class can use this spell
		}

		if (max_level && best_spell_level > max_level) {
			continue;
		}

		if (min_level > 1 && best_spell_level < min_level) {
			continue;
		}
		...
		if (spells[spell_id].spell_group) {
			...
			if (g != spell_group_cache.end()) {
				for (const auto& s : g->second) {
					// Multiclass: check if ANY class can use this spell group member
				uint8 best_sg_level = 255;
				ForEachClass([&](uint8 c) {
					if (!c) return;
					uint8 sl = spells[s].classes[c - 1];
					if (sl > 0 && sl < 255 && sl < best_sg_level) best_sg_level = sl;
				});
				if (
						best_sg_level != 255 &&
						EQ::ValueWithin(best_sg_level, min_level, max_level) &&
						s == spell_id &&
						scribeable
					) {
...
bool Client::CanThisClassTrack() {
	// Multiclass: check if ANY class can track
	uint16 best = 0;
	ForEachClass([&](uint8 c) {
		uint16 mult = GetClassTrackingDistanceMultiplier(c);
