	...
	m_destroying(false)
{
	...
	memset(petids, 0, sizeof(petids));
	SetOwnerID(0);
	SetPetType(PetType::None);
	type_of_pets[1] = 0;
	type_of_pets[2] = 0;
	memset(petpowers, 0, sizeof(petpowers));
	held              = false;
	gheld             = false;
	nocast            = false;
	focused           = false;
...
void Mob::SendHPUpdate(bool force_update_all)
{
	...
	if (IsClient()) {
		if (current_hp != last_hp || force_update_all) {
			...
			CastToClient()->SendEdgeStatPacket();
...
int32 Mob::GetActSpellCasttime(uint16 spell_id, int32 casttime)
{
	...
	if (level > 50 && casttime >= 3000 && !spells[spell_id].good_effect &&
	    (HasAnyClass(Class::Ranger) || HasAnyClass(Class::ShadowKnight) || HasAnyClass(Class::Paladin) || HasAnyClass(Class::Beastlord))) {
