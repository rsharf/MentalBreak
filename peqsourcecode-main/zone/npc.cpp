...
void NPC::PetOnSpawn(NewSpawn_Struct* ns)
{
	...
	LogSpells("PetOnSpawn: name [{}] ownerID [{}] swarmOwner [{}] IsTempPet [{}] petType [{}] spellID [{}]",
		GetName(), GetOwnerID(), GetSwarmOwner(), IsTempPet(), (int)GetPetType(), GetPetSpellID());
	...
	} else if (GetOwnerID() && IsTempPet()) {
		// SE_SummonPet temporary/TargetLock pets (Cleric Hammer, Shaman wolves, etc.)
		// have ownerid set via Pet constructor but no SwarmInfo. Treat like swarm pets
		// so they don't steal the client pet window from the real permanent pet.
		LogSpells("PetOnSpawn: [{}] -> TempPet branch (no is_pet=1)", GetName());
		Mob* temp_owner = entity_list.GetMob(GetOwnerID());
		if (temp_owner) {
			if (temp_owner->IsClient()) {
				SetPetOwnerClient(true);
				SetAllowBeneficial(true);
				ns->spawn.targetable_with_hotkey = false;
				no_target_hotkey = true;
				temp_owner->CastToClient()->Message(Chat::Yellow,
					"DEBUG PetOnSpawn: %s entered TempPet branch (no pet window)", GetCleanName());
			}
			temp_owner->SetTempPetsActive(true);
			temp_owner->SetTempPetCount(temp_owner->GetTempPetCount() + 1);
		}
	} else if (GetOwnerID()) {
		LogSpells("PetOnSpawn: [{}] -> Regular pet branch (is_pet=1)", GetName());
		{
			Mob* dbg_owner = entity_list.GetMob(GetOwnerID());
			if (dbg_owner && dbg_owner->IsClient()) {
				dbg_owner->CastToClient()->Message(Chat::Yellow,
					"DEBUG PetOnSpawn: %s entered REGULAR pet branch (is_pet=1, pet window ON)", GetCleanName());
			}
		}
		ns->spawn.is_pet = 1;
...
bool NPC::IsGuildmasterForClient(Client *c) {
	...
	// Multiclass: check if ANY of the player's classes matches this guildmaster
	for (const auto& [player_class, gm_class] : guildmaster_map) {
		if (gm_class == GetClass() && c->HasAnyClass(player_class)) {
			return true;
		}
	}
