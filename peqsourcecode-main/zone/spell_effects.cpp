...
#define _EDLEN	200
	...
	for (i = 0; i < EFFECT_COUNT; i++)
	{
		...
		switch(effect)
		{
			...
			{
				// Bards don't get mana from effects — but multiclass bards with caster secondary DO
				if(HasAnyClass(Class::Bard) && !IsWisdomCasterClass() && !IsIntelligenceCasterClass())
					break;
...
#endif
				{
				// Check if this is a swarm/temporary pet (e.g., Cleric Hammer)
				bool is_swarm_pet = false;
				PetRecord temp_record;
				if (spell.teleport_zone[0] && content_db.GetPetEntry(spell.teleport_zone, &temp_record)) {
					if (temp_record.petcontrol == PetType::TargetLock || temp_record.temporary) {
						is_swarm_pet = true;
					}
				}

				if (is_swarm_pet) {
					// Route TargetLock/temp spells through TemporaryPets() — the same
					// code path that AA swarm pets use (NPC + SwarmInfo). This avoids
					// creating a Pet object which interferes with the client pet window.
					LogSpells("SE_SummonPet: routing swarm spell [{}] through TemporaryPets", spell_id);
					if (IsClient()) {
						Message(Chat::Yellow, "DEBUG: Routing spell %d through TemporaryPets path", spell_id);
					}
					Mob* spell_target = GetTarget();
					// duration_override=60s, followme=false, sticktarg=true (target lock)
					TemporaryPets(spell_id, spell_target, nullptr, 60, false, true);
				} else {
					int pet_slot = GetPetSlotForSpell(spell_id);
					if(HasPetInSlot(pet_slot))
					{
			...
			case SpellEffect::WeaponProc:
			{
				...
				if (proc_id == SPELL_VAMPIRIC_EMBRACE && HasAnyClass(Class::ShadowKnight)) {
	...
	int ticsremaining, uint16 caster_id)
{
	...
	*/
	...
	if (caster && !caster->HasAnyClass(Class::Bard) && caster->HasBaseEffectFocus()) {
	...
	//This is checked from Mob::ApplySpellBonuses, applied to buffs that receive bonuses. See above, must be in 10% intervals to work.
		...
		if (buff_caster && !buff_caster->HasAnyClass(Class::Bard)) {
			oval = effect_value;
			effect_value = effect_value * static_cast<int>(instrument_mod) / 10;
	...
	*/
	...
	switch(value)
	{
		...
		case IS_CLASS_MELEE_THAT_CAN_BASH_OR_KICK_EXCEPT_BARD:
			if (HasAnyClass(Class::Warrior) || HasAnyClass(Class::Paladin) || HasAnyClass(Class::Ranger) ||
				HasAnyClass(Class::ShadowKnight) || HasAnyClass(Class::Monk) || HasAnyClass(Class::Beastlord) ||
				HasAnyClass(Class::Berserker))
				return true;
			break;

		case IS_CLASS_PURE_MELEE:
			if (HasAnyClass(Class::Rogue) || HasAnyClass(Class::Warrior) || HasAnyClass(Class::Berserker) || HasAnyClass(Class::Monk))
				return true;
			break;
		...
		case IS_CLASS_HYBRID_CLASS:
			if (HasAnyClass(Class::Paladin) || HasAnyClass(Class::Ranger) || HasAnyClass(Class::ShadowKnight) ||
				HasAnyClass(Class::Bard) || HasAnyClass(Class::Beastlord))
				return true;
			break;
		...
		case IS_CLASS_CLERIC:
			if (HasAnyClass(Class::Cleric))
				return true;
			break;

		case IS_CLASS_PALADIN:
			if (HasAnyClass(Class::Paladin))
				return true;
			break;
		...
		case IS_CLASS_SHADOWKNIGHT:
			if (HasAnyClass(Class::ShadowKnight))
				return true;
			break;
		...
		case IS_CLASS_SHAMAN:
			if (HasAnyClass(Class::Shaman))
				return true;
			break;

		case IS_CLASS_NECRO:
			if (HasAnyClass(Class::Necromancer))
				return true;
			break;

		case IS_CLASS_MAGE:
			if (HasAnyClass(Class::Magician))
				return true;
			break;

		case IS_CLASS_ENCHANTER:
			if (HasAnyClass(Class::Enchanter))
				return true;
			break;
		...
		case IS_CLASS_CLR_SHM_DRU:
			if (HasAnyClass(Class::Cleric) || HasAnyClass(Class::Shaman) || HasAnyClass(Class::Druid))
				return true;
			break;

		case IS_CLASS_NOT_WAR_PAL_SK:
			if (!HasAnyClass(Class::Warrior) && !HasAnyClass(Class::Paladin) && !HasAnyClass(Class::ShadowKnight))
				return true;
			break;
		...
		case IS_CLASS_CHAIN_OR_PLATE:
			if (HasAnyClass(Class::Warrior) || HasAnyClass(Class::Bard) || HasAnyClass(Class::ShadowKnight) || HasAnyClass(Class::Paladin) || HasAnyClass(Class::Cleric)
				|| HasAnyClass(Class::Ranger) || HasAnyClass(Class::Shaman) || HasAnyClass(Class::Rogue) || HasAnyClass(Class::Berserker)) {
				return true;
			}
			break;
		...
		case IS_CLASS_KNIGHT_HYBRID_MELEE: {
			bool match = false;
			if (IsClient()) {
				CastToClient()->ForEachClass([&](uint8 c) {
					if (IsHybridClass(c) || IsNonSpellFighterClass(c)) match = true;
				});
			} else {
				match = IsHybridClass(GetClass()) || IsNonSpellFighterClass(GetClass());
			}
		...
		case IS_CLASS_WARRIOR_CASTER_PRIEST: {
			...
			if (IsClient()) {
				CastToClient()->ForEachClass([&](uint8 c) {
					if (IsCasterClass(c) || c == Class::Warrior) match = true;
				});
			} else {
				match = IsCasterClass(GetClass()) || GetClass() == Class::Warrior;
			}
			if (match) return true;
			break;
		}
		...
		case IS_CLIENT_AND_MALE_PLATE_USER:
			if (IsClient() && GetGender() == Gender::Male &&
				(HasAnyClass(Class::Warrior) || HasAnyClass(Class::Cleric) || HasAnyClass(Class::Paladin) || HasAnyClass(Class::ShadowKnight) || HasAnyClass(Class::Bard)))
				return true;
			break;

		case IS_CLEINT_AND_MALE_DRUID_ENCHANTER_MAGICIAN_NECROANCER_SHAMAN_OR_WIZARD:
			if (IsClient() && GetGender() == Gender::Male &&
				(HasAnyClass(Class::Druid) || HasAnyClass(Class::Enchanter) || HasAnyClass(Class::Magician) ||
				 HasAnyClass(Class::Necromancer) || HasAnyClass(Class::Shaman) || HasAnyClass(Class::Wizard)))
				return true;
			break;

		case IS_CLIENT_AND_MALE_BEASTLORD_BERSERKER_MONK_RANGER_OR_ROGUE:
			if (IsClient() && GetGender() == Gender::Male &&
				(HasAnyClass(Class::Beastlord) || HasAnyClass(Class::Berserker) || HasAnyClass(Class::Monk) || HasAnyClass(Class::Ranger) || HasAnyClass(Class::Rogue)))
				return true;
			break;
		...
		case IS_CLASS_CASTER_PRIEST: {
			bool match = false;
			if (IsClient()) {
				CastToClient()->ForEachClass([&](uint8 c) { if (IsCasterClass(c)) match = true; });
			} else {
				match = IsCasterClass(GetClass());
			}
			if (match) return true;
			break;
		}
		...
		case IS_END_OR_MANA_ABOVE_20_PCT: {
			// Multiclass: if character has BOTH fighter and caster classes, check both resources
			if (IsClient() && CastToClient()->GetEndurancePercent() >= 20) {
				return true;
			}
			if (GetManaRatio() >= 20) {
				return true;
		...
		case IS_END_OR_MANA_BELOW_10_PCT: {
			if (IsClient() && CastToClient()->GetEndurancePercent() <= 10) {
				return true;
			}
			if (GetManaRatio() <= 10) {
				return true;
		...
		case IS_END_OR_MANA_BELOW_30_PCT2: {
			if (IsClient() && CastToClient()->GetEndurancePercent() <= 30) {
				return true;
			}
			if (GetManaRatio() <= 30) {
				return true;
