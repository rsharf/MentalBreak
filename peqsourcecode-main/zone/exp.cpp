...
void Client::CalculateStandardAAExp(uint64 &add_aaxp, uint8 conlevel, bool resexp)
{
	...
	if (RuleB(Character, UseRaceClassExpBonuses))
	{
		...
		if (HasAnyClass(Class::Rogue) || HasAnyClass(Class::Warrior)) {
			aatotalmod *= 1.05;
		}
	}
...
void Client::SetEXP(ExpSource exp_source, uint64 set_exp, uint64 set_aaxp, bool isrezzexp, NPC* npc) {
	...
	if (GetLevel() < 20) {
		m_epp.perAA = 0;	// turn off aa exp if they drop below 20
	} else {
...
uint32 Client::GetEXPForLevel(uint16 check_level)
{
	...
	{
		// Multiclass: iterate all classes and use the BEST (lowest) penalty modifier
		float classmod = 1.0;
		if (IsClient()) {
			float best = 1.0;
			CastToClient()->ForEachClass([&](uint8 c) {
				float mod = 1.0;
				if (c == Class::Paladin || c == Class::ShadowKnight || c == Class::Ranger || c == Class::Bard) {
					mod = 1.4;
				} else if (c == Class::Monk) {
					mod = 1.2;
				} else if (c == Class::Wizard || c == Class::Enchanter || c == Class::Magician || c == Class::Necromancer) {
					mod = 1.1;
