...
void ZoneDatabase::SavePetInfo(Client *client)
{
	...
	for (int pet_info_type = PetInfoType::Current0; pet_info_type <= PetInfoType::Suspended2; pet_info_type++) {
...
{
	// Clear all 6 pet info slots (3 current + 3 suspended)
	for (int i = PetInfoType::Current0; i <= PetInfoType::Suspended2; i++) {
		memset(client->GetPetInfo(i), 0, sizeof(PetInfo));
	}
	...
	for (const auto& e : info) {
		if (e.pet < PetInfoType::Current0 || e.pet > PetInfoType::Suspended2) {
			continue;
		}
		p = client->GetPetInfo(e.pet);
