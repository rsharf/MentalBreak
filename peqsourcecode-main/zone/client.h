...
//	void TraderPriceUpdate(const EQApplicationPacket *app);
	...
	// Grant a new multiclass — inserts DB row, sets globals, hydrates, scribes spells
	// Returns 0=success, 1=invalid class, 2=already has class, 3=already at 3 classes, 4=DB error
	int GrantMulticlass(uint8 class_id);
	// How many classes this character has (1-3)
	int GetMulticlassCount() const;
	...
	bool HasAnyClass(uint8 class_id) const override { return HasClass(class_id); }
	bool IsWarriorClass() const override;
	bool IsIntelligenceCasterClass() const override;
	bool IsPureMeleeClass() const override;
	bool IsWisdomCasterClass() const override;
	uint8 GetArchetype() const override;
	uint32 GetClassLevelFactor() override;
	bool CanThisClassTripleAttack() const override;
	bool CanThisClassDualWield(void) const override;
	...
	inline PetInfo* GetPetInfo(int pet_info_type) { if (pet_info_type < 0 || pet_info_type > 5) return &m_petinfos[0]; return &m_petinfos[pet_info_type]; }
	...
	*/
	...
	void UpdatePetXTarget(int pet_slot, Mob *pet);
...
protected:
    ...
    bool                 m_multiclass_loaded = false; // true after HydrateMulticlassFromDB() completes
...
private:
	...
	PetInfo m_petinfos[6]; // indexed by PetInfoType: 0/1=slot0 cur/susp, 2/3=slot1, 4/5=slot2
