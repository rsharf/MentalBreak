...
void Client::OPCombatAbility(const CombatAbility_Struct *ca_atk)
{
	...
	if (
		(spellend_timer.Enabled() && !HasAnyClass(Class::Bard)) ||
		IsFeared() ||
		IsStunned() ||
		IsMezzed() ||
		DivineAura() ||
...
void Client::ThrowingAttack(Mob* other, bool CanDoubleAttack) { //old was 51
	...
	if(!IsAttackAllowed(other) ||
		(IsCasting() && !HasAnyClass(Class::Bard)) ||
		IsSitting() ||
		(DivineAura() && !GetGM()) ||
		IsStunned() ||
		IsFeared() ||
...
void Client::DoClassAttacks(Mob *ca_target, uint16 skill, bool IsRiposte)
{
	...
	if (skill == -1){
		// Multiclass: pick the best class ability from ALL classes
		// Priority: FlyingKick > DragonPunch > EagleStrike > TigerClaw > RoundKick > Backstab > Frenzy > Bash > Kick
		if (HasAnyClass(Class::Monk)) {
			if (GetLevel() >= 30) skill_to_use = EQ::skills::SkillFlyingKick;
			else if (GetLevel() >= 25) skill_to_use = EQ::skills::SkillDragonPunch;
			else if (GetLevel() >= 20) skill_to_use = EQ::skills::SkillEagleStrike;
			else if (GetLevel() >= 10) skill_to_use = EQ::skills::SkillTigerClaw;
			else if (GetLevel() >= 5) skill_to_use = EQ::skills::SkillRoundKick;
			else skill_to_use = EQ::skills::SkillKick;
		}
		else if (HasAnyClass(Class::Rogue)) {
			skill_to_use = EQ::skills::SkillBackstab;
		}
		else if (HasAnyClass(Class::Berserker)) {
			skill_to_use = EQ::skills::SkillFrenzy;
		}
		else if (HasAnyClass(Class::ShadowKnight) || HasAnyClass(Class::Paladin)) {
			skill_to_use = EQ::skills::SkillBash;
		}
		else if (HasAnyClass(Class::Warrior) || HasAnyClass(Class::Ranger) || HasAnyClass(Class::Beastlord)) {
	...
	if (skill_to_use == EQ::skills::SkillFrenzy) {
		...
		if (!IsRiposte && HasAnyClass(Class::Berserker)) {
			int chance = GetLevel() * 2 + GetSkill(EQ::skills::SkillFrenzy);
			if (zone->random.Roll0(450) < chance)
				AtkRounds++;
			if (zone->random.Roll0(450) < chance)
