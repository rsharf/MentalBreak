...
int64 Mob::GetActSpellDamage(uint16 spell_id, int64 value, Mob* target) {
	...
	if (chance > 0 || (IsOfClientBot() && HasAnyClass(Class::Wizard) && GetLevel() >= RuleI(Spells, WizCritLevel))) {
		...
		} else if ((IsOfClientBot() && HasAnyClass(Class::Wizard)) || (IsMerc() && GetClass() == CASTERDPS)) {
		...
		if (IsOfClientBot() && HasAnyClass(Class::Wizard)) {
...
bool Client::TrainDiscipline(uint32 itemid) {
	...
	// Multiclass: only block if ALL of the player's classes are pure casters
	{
		bool has_non_pure_caster = false;
		ForEachClass([&](uint8 c) {
			if (c != Class::Wizard && c != Class::Enchanter && c != Class::Magician && c != Class::Necromancer) {
				has_non_pure_caster = true;
			}
		});
		if (!has_non_pure_caster) {
	...
	//can we use the item? Multiclass: check if any class can use it
	if (!(item->Classes & GetClassesMask())) {
		Message(Chat::Red, "Your class cannot learn from this tome.");
		//summon them the item back...
		SummonItem(itemid);
		return false;
	...
	//can we use the spell? Multiclass: find the lowest valid level across all classes
	const auto& spell = spells[spell_id];
	uint8 level_to_use = 255;
	ForEachClass([&](uint8 c) {
		if (c >= 1 && c <= Class::PLAYER_CLASS_COUNT) {
			uint8 lvl = spell.classes[c - 1];
			if (lvl < level_to_use) {
				level_to_use = lvl;
			}
		}
...
bool Client::MemorizeSpellFromItem(uint32 item_id) {
	...
	// Multiclass: check if any of the player's classes can use this scroll
	if (!(item->Classes & GetClassesMask())) {
	...
	const auto& spell = spells[spell_id];
	// Multiclass: find the lowest valid level across all player classes
	uint8 level_to_use = 255;
	ForEachClass([&](uint8 c) {
		if (c >= 1 && c <= Class::PLAYER_CLASS_COUNT) {
			uint8 lvl = spell.classes[c - 1];
...
bool Client::UseDiscipline(uint32 spell_id, uint32 target) {
	...
	if (spell.recast_time > 0) {
		...
		if (reduced_recast > 0) {
			...
			if (HasAnyClass(Class::Bard) && IsCasting() && spells[spell_id].cast_time == 0) {
	...
	for (auto& it: attacker->GetCloseMobList(distance)) {
		...
		if (
			...
			for (int i = 0; i < attack_rounds; i++) {
				if (
					!attacker->IsClient() ||
					attacker->HasAnyClass(Class::Monk) ||
					attacker->HasAnyClass(Class::Ranger)
				) {
