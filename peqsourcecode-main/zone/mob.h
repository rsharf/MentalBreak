...
public:
	...
	virtual bool HasAnyClass(uint8 class_id) const { return GetClass() == class_id; }
	virtual bool IsWarriorClass() const;
	virtual bool IsIntelligenceCasterClass() const;
	virtual bool IsPureMeleeClass() const;
	virtual bool IsWisdomCasterClass() const;
	virtual uint8 GetArchetype() const;
	...
	inline uint16 GetPetID() const { return petids[0]; }
	inline uint8 GetPetType() const { return type_of_pets[0]; }
	void SetPetType(uint8 pet_type) { type_of_pets[0] = pet_type; }
	inline int16 GetPetPower() const { return (petpowers[0] < 0) ? 0 : petpowers[0]; }
	void SetPetPower(int16 p) { if (p < 0) petpowers[0] = 0; else petpowers[0] = p; }
	bool IsFamiliar() const { return type_of_pets[0] == PetType::Familiar; }
	bool IsAnimation() const { return type_of_pets[0] == PetType::Animation; }
	bool IsCharmed() const { return type_of_pets[0] == PetType::Charmed; }
	bool IsTargetLockPet() const { return type_of_pets[0] == PetType::TargetLock; }
	...
	bool HasPet() const;
	// Multi-pet slot-aware accessors
	inline uint16 GetPetIDInSlot(int slot) const { return (slot >= 0 && slot < 3) ? petids[slot] : 0; }
	inline bool OwnsThisPet(uint16 pet_id) const { return pet_id && (petids[0] == pet_id || petids[1] == pet_id || petids[2] == pet_id); }
	Mob* GetPetInSlot(int slot);
	bool HasPetInSlot(int slot) const;
	void SetPetIDInSlot(int slot, uint16 NewPetID);
	...
	virtual bool CanThisClassDoubleAttack(void) const;
	virtual bool CanThisClassTripleAttack() const;
	virtual bool CanThisClassDualWield(void) const;
	virtual bool CanThisClassRiposte(void) const;
	virtual bool CanThisClassDodge(void) const;
	virtual bool CanThisClassParry(void) const;
	virtual bool CanThisClassBlock(void) const;

	int GetHandToHandDelay(void);
	virtual uint32 GetClassLevelFactor();
	void Mesmerize();
	...
	bool Charmed() const { return type_of_pets[0] == PetType::Charmed; }
