...
void EntityList::AddNPC(NPC *npc, bool send_spawn_packet, bool dont_queue)
{
	...
	if (owner_id) {
		...
		if (owner) {
			// Swarm/temporary pets (e.g., Cleric Hammer, Shaman wolves) should not occupy a pet slot
			if (npc->GetPetType() != PetType::TargetLock && !npc->IsTempPet()) {
				int slot = GetPetSlotForSpell(npc->GetPetSpellID());
				owner->SetPetIDInSlot(slot, npc->GetID());
			}
		}
	}
...
)
{
	...
	LogInfo("[SaylinkDebug] OP_SpecialMesg packet size=[{}] opcode=[0x{:04X}]",
		outapp->size, outapp->GetOpcode());
