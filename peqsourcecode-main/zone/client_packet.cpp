...
void Client::CompleteConnect()
{
    ...
    if (RuleB(Pets, AutoSuspendOnDeath) && !HasPet()) {
        ...
        // if (m_petinfos[PetInfoType::Suspended].SpellID) { UnsuspendMinion(); }
    }
    ...
    LogInfo("[AutoSuspend][Connect-mid] rule={} aabon={} spellbon={} itembon={} SuspendedSpellID={} (valid={})",
            ...
            is_valid_spell(m_petinfos[PetInfoType::Suspended].SpellID));

    if ( !RuleB(Pets, AutoSuspendOnDeath) &&
         !aabonuses.ZoneSuspendMinion &&
         !spellbonuses.ZoneSuspendMinion &&
         !itembonuses.ZoneSuspendMinion )
    {
        for (int ps = 0; ps < 3; ps++) {
            memset(&m_petinfos[PetInfoType::PetInfoSuspended(ps)], 0, sizeof(PetInfo));
        }
    }
    ...
    // Populate pet XTarget entries now that the client is fully connected
    for (int ps = 0; ps < 3; ps++) {
        Mob* slotpet = GetPetInSlot(ps);
        UpdatePetXTarget(ps, slotpet);
    }
...
void Client::Handle_Connect_OP_SendAATable(const EQApplicationPacket *app)
{
	...
	SendMulticlassInfo();
	// SendEdgeStatPacket(); // Moved to Handle_Connect_OP_ClientReady (after zone load completes)
	return;
}
...
{
    // -----------------------------------------------------------------------
    // SERVER-AUTHORITATIVE HANDLER — LOG ONLY, NEVER PERSIST CLIENT DATA
    //
    // The client DLL can send stale class data from a previous character
    // session.  The server's character_classes table (populated exclusively
    // by GrantMulticlass / LoadMultiClassFromDB / HydrateMulticlassFromDB)
    // is the single source of truth.  This handler only logs the client
    // payload for diagnostic purposes and re-sends the correct server-side
    // mask back to the client so the DLL stays in sync.
    // -----------------------------------------------------------------------
    if (!app) {
        return;
    }
    ...
    LogInfo("OP_MulticlassInfo (LOG ONLY): char_id={}, client_classes={}, aa={}, spells={}, skills={}, discs={}, abilities={}, bytes={}",
            hdr->char_id,
            hdr->class_count,
            hdr->aa_count,
            hdr->spell_count,
    ...
    // Decode client-sent class list for logging only
    ...
    // Log what the client thinks vs what the server knows
    std::string client_class_str;
    for (uint16_t i = 0; i < hdr->class_count; ++i) {
        const auto* cc = reinterpret_cast<const THJ::McClass*>(cursor + i * sizeof(THJ::McClass));
        if (i) client_class_str += ",";
        client_class_str += std::to_string(cc->class_id);
    }

    LogInfo("[THJ] OP_MulticlassInfo: Client sent classes=[{}], server mask=0x{:08X}, server primary={}. Client data IGNORED — server is authoritative.",
            client_class_str, GetClassesMask(), GetClass());

    // Re-send the correct server-side mask to the client DLL so it stays in sync
    SendMulticlassInfo();
}
...
void Client::Handle_Connect_OP_ZoneEntry(const EQApplicationPacket *app)
{
	...
	*/
	...
	if (HasClass(Class::ShadowKnight) || HasClass(Class::Paladin)) {
		uint32 abilitynum = 0;
		if (HasClass(Class::ShadowKnight)) { abilitynum = pTimerHarmTouch; }
	...
	if (RuleB(NPC, PetZoneWithOwner)) {
		/* Load Pets — restore all 3 slots */
		database.LoadPetInfo(this);
		for (int ps = 0; ps < 3; ps++) {
			int cur_type = PetInfoType::PetInfoCurrent(ps);
			PetInfo* pi = GetPetInfo(cur_type);
			if (pi->SpellID > 1 && !HasPetInSlot(ps) && pi->SpellID <= SPDAT_RECORDS) {
				MakePoweredPet(pi->SpellID, spells[pi->SpellID].teleport_zone, pi->petpower,
							   pi->Name, pi->size);
				Mob* slotpet = GetPetInSlot(ps);
	...
	// Preserve death-suspended pets across zone only if the rule is enabled
	// AND we actually have a valid snapshot. Otherwise, clear it.
	auto is_valid_spell = [](uint32 sid) {
		return sid > 0 && sid <= SPDAT_RECORDS;
	};

	for (int ps = 0; ps < 3; ps++) {
		int susp_type = PetInfoType::PetInfoSuspended(ps);
		if ( (!RuleB(Pets, AutoSuspendOnDeath) || !is_valid_spell(m_petinfos[susp_type].SpellID)) &&
			!aabonuses.ZoneSuspendMinion &&
			!spellbonuses.ZoneSuspendMinion &&
			!itembonuses.ZoneSuspendMinion )
		{
			memset(&m_petinfos[susp_type], 0, sizeof(PetInfo));
		}
	}
...
void Client::Handle_OP_ApplyPoison(const EQApplicationPacket *app)
{
	...
	if (IsPoison && HasAnyClass(Class::Rogue)) {
...
void Client::Handle_OP_CastSpell(const EQApplicationPacket *app)
{
	...
	if (RuleB(Spells, RequireMnemonicRetention)) {
		// Slots 9-12 (index 8-11): require Mnemonic Retention ranks 1-4
		...
		// Slots 13-16 (index 12-15): require Mnemonic Retention ranks 5-8
		if (EQ::ValueWithin(castspell->slot, 12, 15) && GetAA(aaMnemonicRetention) < (castspell->slot - 7)) {
			InterruptSpell(castspell->spell_id);
			Message(Chat::Red, "You do not have the required AA to use this spell slot.");
			return;
		}
	...
	/* ABILITY cast (LoH and Harm Touch) */
		...
		if (castspell->spell_id == SPELL_LAY_ON_HANDS && HasAnyClass(Class::Paladin)) {
			...
			|| castspell->spell_id == SPELL_HARM_TOUCH2) && HasAnyClass(Class::ShadowKnight)) {
...
void Client::Handle_OP_ItemLinkClick(const EQApplicationPacket *app)
{
	...
	LogInfo("[SaylinkDebug] OP_ItemLinkClick received | item_id=[{}] aug0=[{}] aug1=[{}] hash_dec=[{}] hash_hex=[0x{:08X}]",
		ivrs->item_id, ivrs->augments[0], ivrs->augments[1], ivrs->link_hash, ivrs->link_hash);

	// Handle saylinks before item lookup.
	// Check both SAYLINK_QUEST_ITEM_ID (0xFFFFF, new) and SAYLINK_ITEM_ID (1001, legacy)
	// so links generated before the fix still work.
	if (ivrs->item_id == SAYLINK_QUEST_ITEM_ID || ivrs->item_id == SAYLINK_ITEM_ID) {
		std::string response = "";
...
void Client::Handle_OP_ItemVerifyRequest(const EQApplicationPacket *app)
{
	...
	if (spell_id > 0) {
		...
		if (casting_spell_id ||
			...
			*/
			if (HasAnyClass(Class::Bard) && IsCasting() && casting_spell_slot < CastingSlot::MaxGems)
			{
	...
	if (m_inv.SupportsClickCasting(slot_id) || ((item->ItemType == EQ::item::ItemTypePotion || item->PotionBelt) && m_inv.SupportsPotionBeltCasting(slot_id))) // sanity check
	{
		...
		else if (inst->IsClassCommon())
		{
			...
			{
				if (ClientVersion() >= EQ::versions::ClientVersion::SoD && (!inst->IsRaceEquipable(GetBaseRace()) || !inst->IsClassEquipableByMask(GetClassesMask())))
				{
...
void Client::Handle_OP_PetCommands(const EQApplicationPacket *app)
{
	...
	// Handle PET_LEADER separately (doesn't need per-pet loop)
	...
	// Broadcast command to all active pets across all slots
	for (int pet_slot = 0; pet_slot < 3; pet_slot++) {
	Mob* mypet = GetPetInSlot(pet_slot);
	if (!mypet)
		continue;
...
void Client::Handle_OP_XTargetRequest(const EQApplicationPacket *app)
{
	...
	// Slots 0-2 are reserved for multi-pet health monitoring
	if (Slot < 3)
		return;
