...
void Mob::MakePoweredPet(uint16 spell_id, const char* pettype, int16 petpower,
    ...
    // Swarm/temporary pets (e.g., Cleric Hammer) bypass the multi-pet slot check
    bool is_swarm_pet = (record.petcontrol == PetType::TargetLock || record.temporary);
    int pet_slot = GetPetSlotForSpell(spell_id);
    if(!is_swarm_pet && HasPetInSlot(pet_slot))
        return;
    ...
    // Flag swarm/temporary pets (e.g., Cleric Hammer, Shaman wolves) before
    // AddNPC so PetOnSpawn can treat them like swarm pets (no is_pet=1).
    if (is_swarm_pet) {
        npc->SetTempPet(true);
    }
    ...
    if (!is_swarm_pet) {
        SetPetIDInSlot(pet_slot, npc->GetID());
    }
    ...
    if (record.petcontrol == PetType::TargetLock)
    {
        ...
        LogSpells("TargetLock pet: spell [{}] caster [{}] target [{}] targetID [{}] casterID [{}]",
            spell_id, GetName(),
            m_target ? m_target->GetName() : "NONE",
            m_target ? m_target->GetID() : 0,
            GetID());

        if (IsClient()) {
            Message(Chat::Yellow, "DEBUG TargetLock: your target = %s (id %d), your id = %d",
                m_target ? m_target->GetCleanName() : "NONE",
                m_target ? m_target->GetID() : 0, GetID());
        }
        ...
        if (m_target && m_target->GetID() != GetID()) {
            ...
            if (spells[spell_id].target_type == ST_Self) {
                ...
                LogSpells("TargetLock ST_Self distance check: distance [{}] max [200]", distance);
                if (IsClient()) {
                    Message(Chat::Yellow, "DEBUG TargetLock: distance to %s = %.1f (max 200)",
                        m_target->GetCleanName(), distance);
                }
                if (distance <= 200) {
                    activiate_pet = true;
                }
        ...
        if (activiate_pet){
            ...
            LogSpells("TargetLock pet ACTIVATED on [{}], pet HP [{}]", m_target->GetName(), npc->GetHP());
            if (IsClient()) {
                Message(Chat::Yellow, "DEBUG TargetLock: pet ACTIVATED on %s (pet HP %d)",
                    m_target->GetCleanName(), (int)npc->GetHP());
            }
        }
        else {
            LogSpells("TargetLock pet SELF-DESTRUCT: no valid target");
            if (IsClient()) {
                Message(Chat::Red, "DEBUG TargetLock: pet SELF-DESTRUCT (no valid target within 200 units)");
            }
...
Mob* Mob::GetPetInSlot(int slot) {
    if (slot < 0 || slot >= 3)
        return nullptr;
    uint16 id = petids[slot];
    if (!id)
        return nullptr;
    const auto m = entity_list.GetMob(id);
    if (!m) {
        petids[slot] = 0;
        return nullptr;
    }
...
bool Mob::HasPetInSlot(int slot) const {
    if (slot < 0 || slot >= 3)
        return false;
    uint16 id = petids[slot];
    if (id == 0)
...
void Mob::SetPetIDInSlot(int slot, uint16 NewPetID) {
    if (slot < 0 || slot >= 3)
        return;
    if (NewPetID == GetID() && NewPetID != 0)
        return;
    petids[slot] = NewPetID;

    if (IsClient()) {
        Mob* NewPet = entity_list.GetMob(NewPetID);
        CastToClient()->UpdatePetXTarget(slot, NewPet);
    }
}
