...
int64 Client::CalcHPRegen(bool bCombat)
{
	...
	int64 base = 0;
	// Multiclass: use the BEST (highest) HP regen from all classes
	ForEachClass([&](uint8 c) {
		auto bd = zone->GetBaseData(GetLevel(), c);
		if (bd.level == GetLevel()) {
			int64 candidate = static_cast<int>(bd.hp_regen);
...
int64 Client::CalcBaseHP()
{
	if (ClientVersion() >= EQ::versions::ClientVersion::SoF && RuleB(Character, SoDClientUseSoDHPManaEnd)) {
		...
		base_hp = 5;
		// Multiclass: iterate all classes and use the BEST base HP
		double best_hp = 0;
		double best_hp_fac = 0;
		bool found = false;
		ForEachClass([&](uint8 c) {
			auto bd = zone->GetBaseData(GetLevel(), c);
			if (bd.level == GetLevel()) {
				double candidate = bd.hp + (bd.hp_fac * stats);
				if (!found || candidate > best_hp) {
					best_hp = candidate;
					best_hp_fac = bd.hp_fac;
					found = true;
				}
			}
		});
		if (!found) {
			...
			auto base_data = zone->GetBaseData(GetLevel(), GetClass());
			if (base_data.level == GetLevel()) {
				best_hp = base_data.hp + (base_data.hp_fac * stats);
				found = true;
			}
		}
		if (found) {
			base_hp += static_cast<int64>(best_hp);
...
{
	// Multiclass: check if ANY class (primary or secondary) is a caster
	bool has_caster = IsIntelligenceCasterClass() || IsWisdomCasterClass();
	if (!has_caster) {
		for (uint8 sec : GetSecondaryClasses()) {
			switch (sec) {
				case Class::ShadowKnight: case Class::Bard: case Class::Necromancer:
	...
	// Multiclass: if character just gained a caster class, current_mana may be 0
	// from when they were a pure non-caster. Fill mana pool on first calculation.
	if (max_mana > 0 && current_mana == 0) {
...
{
	// Multiclass: evaluate each class and pick the one that yields the most mana.
	// INT casters: ShadowKnight, Bard, Necromancer, Wizard, Magician, Enchanter
	// WIS casters: Cleric, Paladin, Ranger, Druid, Shaman, Beastlord

	auto calc_mana_for_class = [&](uint8 cls) -> int64 {
		bool is_int = false, is_wis = false;
		switch (cls) {
			case Class::ShadowKnight: case Class::Bard: case Class::Necromancer:
			case Class::Wizard: case Class::Magician: case Class::Enchanter:
				is_int = true; break;
		...
		int WisInt = is_int ? GetINT() : GetWIS();

		if (ClientVersion() >= EQ::versions::ClientVersion::SoF && RuleB(Character, SoDClientUseSoDHPManaEnd)) {
			int ConvertedWisInt = WisInt;
			int over200 = WisInt;
			if (WisInt > 100) {
				if (WisInt > 200) {
			...
			auto base_data = zone->GetBaseData(GetLevel(), cls);
			if (base_data.level == GetLevel()) {
				return base_data.mana + (ConvertedWisInt * base_data.mana_fac) + itembonuses.heroic_max_mana;
			}
			return 0;
		} else {
			int MindLesserFactor = ((WisInt - 199) / 2) > 0 ? (WisInt - 199) / 2 : 0;
			int MindFactor = WisInt - MindLesserFactor;

			if (WisInt > 100) {
				return (((5 * (MindFactor + 20)) / 2) * 3 * GetLevel() / 40);
			} else {
				return (((5 * (MindFactor + 200)) / 2) * 3 * GetLevel() / 100);
			}
		}
	};

	int64 best = calc_mana_for_class(GetClass());

	for (uint8 sec : GetSecondaryClasses()) {
		int64 m = calc_mana_for_class(sec);
		if (m > best) {
			best = m;
		}
	}
...
int32	Client::CalcMR()
{
	...
	if (HasAnyClass(Class::Warrior) || HasAnyClass(Class::Berserker)) {
		MR += GetLevel() / 2;
	}
	if (MR < 1) {
		MR = 1;
...
int32	Client::CalcDR()
{
	...
	if (HasAnyClass(Class::ShadowKnight) || HasAnyClass(Class::Beastlord)) {
		DR += 4;
		int l = GetLevel();
		if (l > 49) {
			DR += l - 49;
...
int32	Client::CalcCR()
{
	...
	if (HasAnyClass(Class::Ranger) || HasAnyClass(Class::Beastlord)) {
		CR += 4;
		int l = GetLevel();
		if (l > 49) {
			CR += l - 49;
...
{
	if (!HasAnyClass(Class::Bard) || spells[spell_id].is_discipline || spell_id == SPELL_AMPLIFICATION) {
...
int64 Client::CalcBaseEndurance()
{
	...
	if (ClientVersion() >= EQ::versions::ClientVersion::SoF && RuleB(Character, SoDClientUseSoDHPManaEnd)) {
		...
		// Multiclass: use the BEST base endurance from all classes
		bool end_found = false;
		ForEachClass([&](uint8 c) {
			auto bd = zone->GetBaseData(GetLevel(), c);
			if (bd.level == GetLevel()) {
				int64 candidate = static_cast<int64>(bd.end + itembonuses.heroic_max_end + (bd.end_fac * static_cast<int>(stats)));
				if (!end_found || candidate > base_end) {
					base_end = candidate;
					end_found = true;
				}
			}
...
int64 Client::CalcEnduranceRegen(bool bCombat)
{
	int64 base = 0;
	if (!IsStarved()) {
		// Multiclass: use the BEST (highest) endurance regen from all classes
		ForEachClass([&](uint8 c) {
			auto bd = zone->GetBaseData(GetLevel(), c);
			if (bd.level == GetLevel()) {
				int64 candidate = static_cast<int>(bd.end_regen);
				if (candidate > base) base = candidate;
			}
		});
		if (base == 0) {
