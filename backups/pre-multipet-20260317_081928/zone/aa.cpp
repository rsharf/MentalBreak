...
void Client::SendAlternateAdvancementRank(int aa_id, int level) {
	...
	{
		bool aa_class_ok = false;
		ForEachClass([&](uint8 c) { if (ability->classes & (1 << c)) aa_class_ok = true; });
		if (!aa_class_ok) return;
	}
	...
	// Include primary class bit so the client's AA window displays AAs from
	// all multiclass classes, not just the primary.  The AA encoding uses
	// (1 << class_id) where class_id is 1-16.  ORing in the primary class
	// bit is a no-op for AAs that already belong to the primary class.
	aai->classes = ability->classes | (1 << GetClass());
...
void Client::ActivateAlternateAdvancementAbility(int rank_id, int target_id) {
	...
	if (use_toggle_passive_hotkey) {
		...
		if (HasAnyClass(Class::Bard) && IsCasting() && spells[rank->spell].cast_time == 0) {
...
bool Mob::CanUseAlternateAdvancementRank(AA::Rank *rank)
{
...

	{
		bool aa_class_ok = false;
		if (IsClient()) {
			CastToClient()->ForEachClass([&](uint8 c) { if (a->classes & (1 << c)) aa_class_ok = true; });
		} else {
			aa_class_ok = (a->classes & (1 << GetClass())) != 0;
		}
		if (!aa_class_ok) return false;
	}
