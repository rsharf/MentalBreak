...
void Client::OPMemorizeSpell(const EQApplicationPacket* app)
{
	...
	if (m->scribing != memSpellForget && IsPlayerClass(GetClass())) {
		// Check primary class first
		bool can_use = (GetLevel() >= spells[m->spell_id].classes[GetClass() - 1] &&
		                spells[m->spell_id].classes[GetClass() - 1] < 255);

		// Check multiclass secondary classes
		if (!can_use) {
			for (uint8 sec : GetSecondaryClasses()) {
				if (sec >= 1 && sec <= 16 &&
				    spells[m->spell_id].classes[sec - 1] < 255 &&
				    GetLevel() >= spells[m->spell_id].classes[sec - 1]) {
					can_use = true;
					break;
				}
			}
	...
	} else if (m->scribing != memSpellForget && !IsPlayerClass(GetClass())) {
		MessageString(
			Chat::Red,
			SPELL_LEVEL_TO_LOW,
			std::to_string(spells[m->spell_id].classes[GetClass() - 1]).c_str(),
		...
		case memSpellScribing: {
			...
			if (inst && inst->IsClassCommon()) {
				...
				if (
					...
					(!item->IsRaceEquipable(GetRace()) || !item->IsClassEquipableByMask(GetClassesMask()))
				) {
...
//#pragma GCC optimize ("O0")
	for (int sk = EQ::skills::Skill1HBlunt; sk <= EQ::skills::HIGHEST_SKILL; ++sk) {
		if (sk == EQ::skills::SkillTinkering && GetRace() != Race::Gnome) {
			...
			// Multiclass: use merged skill cap (best across all classes) instead of primary-only
			gmtrain->skills[sk] = GetMaxSkillAfterSpecializationRules((EQ::skills::SkillType)sk, MergedSkillCap((EQ::skills::SkillType)sk, static_cast<uint8>(RuleI(Character, MaxLevel))));
	...
	if (ClientVersion() < EQ::versions::ClientVersion::RoF2 && HasAnyClass(Class::Berserker)) {
	...
	else if (gmskill->skillbank == 0x00)
	{
		...
		if (skilllevel == 0) {
			...
			ForEachClass([&](uint8 c) {
				if (!c) return;
				uint16 tl = GetSkillTrainLevel(skill, c);
				if (tl > 0 && (t_level == 0 || tl < t_level)) t_level = tl;
			});
