...
// Multiclass mask received from server (bit N-1 set = class N enabled)
uint32_t g_multiclass_mask = 0;
...
{
	// Do NOT clear statEntries here.  EQ_END_ZONE fires AFTER the server's
	// ClientReady handler sends 0x1338, so clearing wipes the data we just
	// received.  The server sends fresh stats on every zone-in, so stale
...
// --- Skill Activated filtering ---
// The Actions window dropdown shows all skills where _SKILL::Activated is true.
// The vanilla RoF2 client sets Activated for ALL combat abilities regardless of class.
// We fix this by setting Activated = false for skills the character's class(es) can't use.
static bool g_originalActivatedSaved = false;
static bool g_originalActivated[NUM_SKILLS] = { false };
static uint32_t g_lastFilteredMask = 0;

void FilterSkillActivatedByClass()
{
	if (!pCSkillMgr)
		return;
	...
	// Build effective class mask
	uint32_t mask = g_multiclass_mask;
	if (mask == 0) {
		DWORD classId = pCI2->Class;
	...
	// Skip if already filtered for this exact mask
	if (g_lastFilteredMask == mask && g_originalActivatedSaved)
		return;

	// Save original Activated values once (before we ever modify them)
	if (!g_originalActivatedSaved) {
		for (int i = 0; i < NUM_SKILLS; i++) {
			PSKILL pSkill = ((PSKILLMGR)pCSkillMgr)->pSkill[i];
			g_originalActivated[i] = (pSkill && pSkill->Activated);
		}
		g_originalActivatedSaved = true;
	}
	...
	for (int i = 0; i < NUM_SKILLS; i++) {
		...
		// Restore original value first (handles class/character changes)
		pSkill->Activated = g_originalActivated[i];

		// If not originally activated, nothing to filter
		if (!g_originalActivated[i])
			continue;

		// Check if any class in the mask can use this skill.
		// Available[c] != 0xFF means the skill slot exists for that class,
		// but MinLevel[c] > 0 is the REAL indicator of class availability.
		bool canUse = false;
		for (int c = 1; c <= 16; c++) {
			if (mask & (1u << (c - 1))) {
				if (c < 0x24 && pSkill->Available[c] != 0xFF && pSkill->MinLevel[c] > 0) {
					canUse = true;
					break;
				}
			}
		...
		if (!canUse) {
			pSkill->Activated = false;
		}
	}

	g_lastFilteredMask = mask;
}

PLUGIN_API BOOL OnRecvMulticlassMaskPacket(DWORD Type, PVOID Packet, DWORD Size)
{
	if (Size >= 4 && Packet) {
		g_multiclass_mask = *(uint32_t*)Packet;

		// Re-filter skill Activated flags with updated mask
		g_lastFilteredMask = 0; // force re-filter
		FilterSkillActivatedByClass();

		// Scrub ability list of skills not valid for any class in the mask
		if (EQADDR_DOABILITYLIST && pCSkillMgr) {
			for (int i = 0; i < 10; i++) {
				DWORD skillId = EQADDR_DOABILITYLIST[i];
				if (skillId != 0xFFFFFFFF && skillId < NUM_SKILLS) {
					if (!pCSkillMgr->IsAvailable(skillId)) {
						EQADDR_DOABILITYLIST[i] = 0xFFFFFFFF;
					}
				}
			}
		}
...
public:
	...
	int32_t MaxMana_Detour(int a1)
	{
		...
		// Fall through to the real game function so multiclass mana pool is correct
	...
	int32_t CurMana_Detour(int a1)
	{
		...
		// Fall through to the real game function so multiclass mana is correct
	...
	int GetSpellLevelNeeded_Trampoline(int class_id);
	int GetSpellLevelNeeded_Detour(int class_id)
	{
		// If we have a multiclass mask, find the lowest level across all
		// of the player's classes so the spellbook shows the real level.
		if (g_multiclass_mask != 0) {
			int best = 255;
			for (int c = 1; c <= 16; c++) {
				if (g_multiclass_mask & (1u << (c - 1))) {
					int lvl = GetSpellLevelNeeded_Trampoline(c);
					if (lvl > 0 && lvl < 255 && lvl < best) {
						best = lvl;
					}
				}
			}
		...
		return GetSpellLevelNeeded_Trampoline(class_id);
	}
	...
	// ---------------------------------------------------------------
	// Multiclass skill hooks
	// ---------------------------------------------------------------

	// CSkillMgr::IsAvailable(int skillId)
	// When multiclass is active, REPLACES the original logic entirely.
	// Only returns true if the skill is available for a class in the mask.
	bool IsSkillAvailable_Trampoline(int skillId);
	bool IsSkillAvailable_Detour(int skillId)
	{
		if (skillId < 0 || skillId >= NUM_SKILLS)
			return false;

		PSKILL pSkill = pSkillMgr->pSkill[skillId];
		if (!pSkill)
			return false;

		PCHARINFO2 pCI2 = GetCharInfo2();
		if (!pCI2)
			return IsSkillAvailable_Trampoline(skillId);

		// Ensure Activated flags are filtered for this character's class(es).
		// No-ops after the first call for a given mask thanks to g_lastFilteredMask guard.
		FilterSkillActivatedByClass();

		// Build effective class mask: multiclass mask if set, otherwise primary class
		uint32_t mask = g_multiclass_mask;
		if (mask == 0) {
			DWORD classId = pCI2->Class;
			if (classId >= 1 && classId <= 16)
		...
		DWORD playerLevel = pCI2->Level;
		for (int c = 1; c <= 16; c++) {
			if (mask & (1u << (c - 1))) {
				if (c < 0x24 && pSkill->Available[c] != 0xFF && pSkill->MinLevel[c] > 0) {
					if (pSkill->MinLevel[c] <= playerLevel) {
						return true;
					}
				}
			}
	...
	// CSkillMgr::GetSkillCap(EQ_Character*, int level, int classId,
	//                        int skillId, bool, bool, bool)
	// Returns the highest skill cap across all classes in the multiclass mask.
	unsigned long GetSkillCap_Trampoline(class EQ_Character*, int, int, int, bool, bool, bool);
	unsigned long GetSkillCap_Detour(class EQ_Character* pChar, int level, int classId,
	                                 int skillId, bool a5, bool a6, bool a7)
	{
		// Build effective class mask: multiclass mask if set, otherwise primary class
		uint32_t mask = g_multiclass_mask;
		if (mask == 0) {
			PCHARINFO2 pCI2 = GetCharInfo2();
			if (pCI2 && pCI2->Class >= 1 && pCI2->Class <= 16)
				mask = 1u << (pCI2->Class - 1);
			else
				return GetSkillCap_Trampoline(pChar, level, classId, skillId, a5, a6, a7);
		}

		unsigned long bestCap = 0;
		for (int c = 1; c <= 16; c++) {
			if (mask & (1u << (c - 1))) {
				unsigned long cap = GetSkillCap_Trampoline(pChar, level, c, skillId, a5, a6, a7);
				if (cap > bestCap) {
					bestCap = cap;
				}
			}
	...
	// CharacterZoneClient::HasSkill(int skillId)
	// When multiclass is active, REPLACES the original logic entirely.
	// Only returns true if the skill is valid for a class in the mask.
	bool HasSkill_Trampoline(int skillId);
	bool HasSkill_Detour(int skillId)
	{
		if (skillId < 0 || skillId >= NUM_SKILLS)
			return false;
		...
		PCHARINFO2 pCI2 = GetCharInfo2();
		if (!pSkill || !pCI2)
			return HasSkill_Trampoline(skillId);
	...
	// PcZoneClient::GetPcSkillLimit(int skillId)
	// Returns the highest skill limit across all classes in the multiclass mask.
	// Leverages the already-hooked CSkillMgr::GetSkillCap which merges caps
	// across all multiclass classes, so we just need to call it once.
	int GetPcSkillLimit_Trampoline(int skillId);
	int GetPcSkillLimit_Detour(int skillId)
	{
		PCHARINFO pCharInfo = GetCharInfo();
		PCHARINFO2 pCI2 = GetCharInfo2();
		if (!pCharInfo || !pCI2) {
			return GetPcSkillLimit_Trampoline(skillId);
		}

		// GetSkillCap goes through our hook which builds the effective
		// class mask (multiclass or primary) and iterates classes.
		unsigned long mergedCap = pCSkillMgr->GetSkillCap(
			(EQ_Character*)pCharInfo, pCI2->Level, pCI2->Class, skillId,
			true, true, true);

		return (int)mergedCap;
	}
...
DETOUR_TRAMPOLINE_EMPTY(bool EQCharacter1Hook::IsSkillAvailable_Trampoline(int));
DETOUR_TRAMPOLINE_EMPTY(unsigned long EQCharacter1Hook::GetSkillCap_Trampoline(class EQ_Character*, int, int, int, bool, bool, bool));
DETOUR_TRAMPOLINE_EMPTY(bool EQCharacter1Hook::HasSkill_Trampoline(int));
DETOUR_TRAMPOLINE_EMPTY(int EQCharacter1Hook::GetPcSkillLimit_Trampoline(int));
...
PLUGIN_API VOID InitializeMQ2Labels(VOID)
{
 //   DebugSpewAlways("Initializing MQ2Labels");
	...
	EzDetour(CSkillMgr__IsAvailable, &EQCharacter1Hook::IsSkillAvailable_Detour, &EQCharacter1Hook::IsSkillAvailable_Trampoline);
	EzDetour(CSkillMgr__GetSkillCap, &EQCharacter1Hook::GetSkillCap_Detour, &EQCharacter1Hook::GetSkillCap_Trampoline);
	EzDetour(CharacterZoneClient__HasSkill, &EQCharacter1Hook::HasSkill_Detour, &EQCharacter1Hook::HasSkill_Trampoline);
	EzDetour(PcZoneClient__GetPcSkillLimit, &EQCharacter1Hook::GetPcSkillLimit_Detour, &EQCharacter1Hook::GetPcSkillLimit_Trampoline);
...
PLUGIN_API VOID ShutdownLabelsPlugin(VOID)
{
   // DebugSpewAlways("Shutting down MQ2Labels");
    ...
    RemoveDetour(CSkillMgr__IsAvailable);
    RemoveDetour(CSkillMgr__GetSkillCap);
    RemoveDetour(CharacterZoneClient__HasSkill);
    RemoveDetour(PcZoneClient__GetPcSkillLimit);
