...
public:
	...
	HRESULT WINAPI EndScene_Detour()
	{
		...
		if (g_deviceAcquired)
		{
			// Late-bind AA window hooks: pAAWnd is NULL at init time,
			// so we retry each frame until the player enters the world
			// and the AA window is created.
			InstallAAWindowHooks();
